...
首頁（`app/page.jsx`）為 Server Component（`revalidate=60`），價格與 CTA 文案由 `sale_settings` 表即時決定：開課日（`open_at`）、早鳥截止日（`early_bird_ends_at`）、各方案 `plan_pricing[plan].original` / `plan_pricing[plan].earlyBird`、`lock_override`（手動覆寫鎖站/開課）、`launch_notified_at`（開課通知是否已送出）。後台「銷售設定」tab（在 `/admin` 內）可即時修改、**免重新部署**；API 端點為 `/api/admin/sale-settings`。首頁顯示早鳥倒數與優惠價；開課後切換至一般售價。購買前需先登入（`startBuy` 未登入會導向 `/classroom/login`）。

> 價格權威來源在後端 `lib/sale.js` 的 `currentPrice(plan, settings, now)`（checkout 不信任前端傳入的 price）。舊 `NEXT_PUBLIC_PRESALE_MODE` 環境變數已全面移除，不再使用。
...
| `/api/admin/sale-settings` | GET/PATCH | 後台銷售設定（open_at / early_bird_ends_at / plan_pricing / lock_override）|
