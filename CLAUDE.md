...
- **優惠券**：`coupons` 表 + `lib/plans.js`（`applyCoupon` 算折後價、`couponError` 驗證）。`BuyModal` 先打 `/api/coupons/validate` 顯示折扣，checkout **後端再驗證**並寫入 `orders.coupon_code`；`used` 於 notify 付款成功時才 +1。
