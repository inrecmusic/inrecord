...
首頁（`app/page.jsx`）為 Server Component（`revalidate=60`），價格與 CTA 文案由 `sale_settings` 表即時決定：開課日（`open_at`）、早鳥截止日（`early_bird_ends_at`）、各方案 `original_price` / `early_bird_price`、`lock_override`（手動覆寫鎖站/開課）、`launch_notified_at`（開課通知是否已送出）。後台「銷售設定」頁（`/admin/sale-settings`）可即時修改、**免重新部署**。首頁顯示早鳥倒數與優惠價；開課後切換至一般售價。購買前需先登入（`startBuy` 未登入會導向 `/classroom/login`）。
