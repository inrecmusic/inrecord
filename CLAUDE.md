...
- **早鳥搶先看分層**（`lib/early-access.js`）：`EARLY_CUTOFF_MS`＝2026-09-02 00:00（台灣）之前付款／開通者為早鳥（音樂會預購），跟著每週上架進度看、不受限；9/2 起購課者**分兩批**放行（對外電子報承諾）——`FULL_RELEASE_MS`＝9/30 20:00 起開放**第一批 Ch1～Ch3**、`SECOND_RELEASE_MS`＝10/31 20:00 起**完整上架**，之前正課影片全擋。可播與否由純函式 `canPlayVideo()` 統一判斷（bootstrap 與 video-embed 共用，不各寫一份）：第一批看 `chapters.sort_order` ≤ `FIRST_BATCH_MAX_SORT`（＝2）——⚠️ **正式庫 sort_order 是 0-based（0＝Ch1），寫成 1..3 會變成 Ch2～Ch4**，附錄（10／11）不屬於第一批。bootstrap 摘掉可播欄位（fail-open）、video-embed fail-closed 硬閘門（章節查不到時對非早鳥保守擋下、早鳥不受影響）；試看單元（標題以「試看」開頭）任何時候都不受限。例外用 `enrollments.early_override`（'early'|'standard'）。⚠️ 純手動開通且無 9/2 前訂單紀錄者會被判非早鳥，需設 override。
...
- **首頁留信箱／潛客名單**（2026-09）：首頁 FAQ 之後的 `components/LeadCapture.jsx`（勾選同意才能送）→ `POST /api/newsletter/subscribe` → `lib/brevo-contacts.js` 加進 Brevo 清單 `BREVO_LIST_ID`（屬性 SOURCE／CONSENT_AT／UTM_*，Brevo 缺屬性會自動去屬性重試）並送 Lead 事件。名單只存 Brevo、不建表。**進名單後立刻寄「免費試看」信**（`lib/trial.js`：Email 專屬 HMAC 連結 scope `trial:`，與退訂簽章不互通）→ `/trial?e=&t=` 驗簽章後播 `site_content.trial_video_id`（後台「銷售設定 → 免費試看影片」填 Bunny GUID、免部署；空＝顯示準備中）的 Bunny 簽名 embed；簽章無效顯示留信箱表單。首頁另有進站彈窗 `components/LeadPopup.jsx`（停留 6 秒或捲到一半；關掉 7 天不彈、已留過／已登入不彈，localStorage）。後台電子報第三個對象「潛客名單」＝清單全部聯絡人 − 已購買（buyers）− 已退訂；學員按退訂連結時同步從 Brevo 清單移除（best-effort）。
- **營運助理週報**（2026-09）：`lib/ops-report/`——`period`（台灣時間週一切）、`collect`＋`load`（資料包：訂單／待開通／學員觀看／電子報／廣告／優惠券異常／內容進度／近期時程，**數字全由程式算**）、`generate`（`claude-opus-5`、adaptive thinking、effort medium、`output_config.format` json_schema、system 加 cache_control）、`email`（摘要信）、`run`（cron 冪等：同 period_end 只一份；手動用過去 7 天不寄信）。路由 `/api/cron/ops-report`（Bearer CRON_SECRET、每週一 00:00 UTC）、`/api/admin/ops-report`（GET）、`/api/admin/ops-report/run`（POST，10 分鐘一次）。後台頁 `app/admin/OpsAssistantPage.jsx`，建議的 `admin_path` 只接受 `ADMIN_PAGES` 白名單。**完全唯讀**，模型沒有工具。
...
- **教室公告**（2026-09 改版）：`announcements`（title/body/pinned/important/published）由 bootstrap 一併帶回。UI 集中在 `components/Announcements.jsx`（`useAnnouncements` hook）：儀表板「最新公告」區（最多 3 則、可展開）＋播放頁「鈴鐺＋未讀數／未讀提示條／右側抽屜」；`important` 的公告進儀表板或播放頁會先彈卡片、需按「知道了」（每則一次）。**沒有公告時全部不渲染。** 已讀／已確認記在該裝置 localStorage（`lib/announcement-state.js`），不跨裝置。內容為受限 Markdown（`lib/announcement-md.js`：共用電子報 `mdToHtml` plain 模式＋裸網址自動連結，已跳脫）。排序／未讀／挑選純函式在 `lib/announcements-view.js`。不寄信、無排程下架。
...
新環境依序執行 **`supabase-schema-music.sql`**（chapters／videos 是全站地基，**必須第一份**）→ `supabase-schema.sql`（course_preview_leads／orders／games；games 參照 chapters／videos）→ **`supabase-schema-core.sql`**（enrollments／subscriptions 核心表，原本只存在於正式站、repo 沒有 DDL）→ **`supabase-schema-classroom.sql`**（⚠️**一定要排在 music 之後**：它 DROP 掉 music 的舊版 ratings／submissions／unit_comments，重建成程式實際在用的版本 `ratings.score`／`user_id`；順序顛倒＝評價功能整組壞。⚠️ 本檔含 `DROP TABLE ... CASCADE`，**只給全新環境**，對有資料的庫重跑會刪光留言／評價／作業繳交）→ **`supabase-deploy.sql`**（彙整發票欄位 / coupons 表 / courses 表 / **`sale_settings` 表**，idempotent 可重複執行）→ **`supabase-classroom-features.sql`**（教室七大功能：`materials` 表＋`course-materials` 私有 bucket、`announcements` 表…；自帶 RLS service_role policy，分段 idempotent）→ **`supabase-tracking.sql`**（廣告追蹤碼中心：`tracking_settings` 表 + `orders.attribution` 欄，自帶 RLS service_role policy，idempotent）→ **`supabase-recovery.sql`**（未成交挽回信：`orders.recovery_sent_at` 欄，idempotent）→ **`supabase-ad-insights.sql`**（廣告成效儀錶板：`ad_insights` 表，自帶 RLS service_role policy，idempotent）→ **`supabase-capi.sql`**（Meta CAPI：`orders.capi_data` 欄，idempotent；⚠️checkout 每筆都寫此欄，**漏跑會讓所有結帳失敗**）→ **supabase-student-profiles.sql**（學員資料頁：student_profiles 表，自帶 RLS service_role policy，idempotent）→ **supabase-game-security.sql**（互動遊戲安全：game_devices／game_settings 表，自帶 RLS service_role policy，idempotent）→ **supabase-newsletter-unsubscribe.sql**（電子報退訂名單 `newsletter_unsubscribes`，自帶 RLS service_role policy，idempotent；未跑時群發照常 fail-open、但學員點退訂會報錯）→ **supabase-announcements-important.sql**（教室公告「重要」旗標 `announcements.important`，idempotent；⚠️未跑時後台新增／編輯公告與教室載入公告會因欄位不存在而失敗）→ **supabase-terms-consent.sql**（結帳二次確認：`orders.terms_version`／`terms_agreed_at`，idempotent；⚠️checkout 每筆都寫這兩欄，**漏跑會讓所有結帳失敗**）→ **supabase-ops-reports.sql**（營運助理週報 `ops_reports`，自帶 RLS service_role policy，idempotent；未跑時週報寫入失敗但不影響其他功能）→ **supabase-payment-events.sql**（PAYUNi 原始回呼落地 `payment_events` ＋ `orders.refunded_at`／`refund_amount`，自帶 RLS service_role policy，idempotent；未跑時全面安全降級——notify 照常付款只是不留底、退款退回只寫 status、試算表同步退回以 updated_at 推算，詳見 `docs/payment-durability.md`）→ **supabase-classroom-rls-tighten.sql**（收掉 comments／ratings／submissions 對登入帳號的直連 policy；前提是 classroom API 已走 service role，idempotent；⚠️先部署程式再跑）→ **supabase-progress-lock.sql**（收掉 `upsert_progress` RPC 對 anon／authenticated 的 EXECUTE、`progress` 表只留 service_role 全權＋登入者唯讀自己；不依賴程式版本、隨時可跑，idempotent）→ **`supabase-hardening.sql`（必跑，最後一步）**。

> 這串順序與 `docs/preview-db/build-schema.sh` 的 `ORDER` 陣列一致，**改一邊要兩邊一起改**。

> **Preview 專用資料庫**：preview 與正式站原本共用 Supabase；隔離步驟見 `docs/preview-db/README.md`（建專案 → `build-schema.sh` 合併 SQL 一次貼 → `seed.sql` → Vercel Preview 三個 Supabase 變數指過去）。
