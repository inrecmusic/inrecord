...
export default function ProfileFields({ prof, setProf, styles }) {
  ...
  return (
    <>
      <div><label style={label} htmlFor="prof-real_name">真實姓名 *</label>
        <input id="prof-real_name" style={input} value={prof.real_name} onChange={e => setProf(p => ({ ...p, real_name: e.target.value }))} /></div>
      <div><label style={label} htmlFor="prof-phone">手機 *</label>
        <input id="prof-phone" style={input} value={prof.phone} onChange={e => setProf(p => ({ ...p, phone: e.target.value }))} placeholder="09xxxxxxxx" /></div>
      ...
      <div><label style={label} htmlFor="prof-source">怎麼認識 InRecord（選填）</label>
        <select id="prof-source" style={input} value={prof.source} onChange={e => setProf(p => ({ ...p, source: e.target.value }))}>
