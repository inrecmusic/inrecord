...
import { isTrialVideo, canPlayVideo, FULL_RELEASE_MS, SECOND_RELEASE_MS } from "@/lib/early-access";
...
export async function GET(req) {
  ...
  const { data: video } = await supabase
    .from("videos")
    .select("title, chapter_id, bunny_video_id, vimeo_id")
    .eq("id", videoId)
    .eq("published", true) // 只簽發已發布影片，擋已購課者猜 UUID 取未發布草稿的簽名網址
    .maybeSingle();
  if (!video) return NextResponse.json({ error: "video_not_found" }, { status: 404 });

  // 早鳥搶先看分層：9/30 前非早鳥不簽發正課影片、9/30–10/31 只簽發第一批（Ch1～Ch3）、10/31 起全開（試看單元不限）。
  // bootstrap 已在 UI 層摘掉可播欄位，這裡是防「直接打 API 帶單元 id」繞過的硬性閘門。
  // fail-closed：resolveEarlyAccess 查詢故障時回 early=false，寧可暫時擋住也不讓硬閘門失效。
  const now = Date.now();
  if (now < SECOND_RELEASE_MS && !isTrialVideo(video)) {
    const { early } = await resolveEarlyAccess(supabase, user.email);
    let chapterSort; // 查不到章節（chapter_id 為 null 或查詢失敗）→ 留 undefined，對非早鳥保守擋下；早鳥不受影響
    if (!early && now >= FULL_RELEASE_MS && video.chapter_id) {
      // 網路層例外會讓整支 route 變成 500；包起來退回 undefined，維持「查不到就擋」的保守語意
      const ch = await supabase.from("chapters").select("sort_order").eq("id", video.chapter_id).maybeSingle()
        .then((r) => r?.data)
        .catch((e) => { console.error("[video-embed] 章節查詢失敗，保守擋下", e?.message || e); return null; });
      chapterSort = ch?.sort_order;
    }
    if (!canPlayVideo({ video, early, nowMs: now, chapterSort })) {
      // 9/30–10/31 這段期間，早鳥每次播 Ch4 以後都要先查早鳥資格；查詢瞬斷會讓真早鳥吃到 403。
      // 日誌上「正常擋非早鳥」與「早鳥被誤擋」長得一樣，這行是唯一能分辨的線索。
      console.error("[video-embed] 403 not_released", { email: user.email, videoId, early, chapterSort });
      return NextResponse.json({ error: "not_released" }, { status: 403 });
    }
  }
