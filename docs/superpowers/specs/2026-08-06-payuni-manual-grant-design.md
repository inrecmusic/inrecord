...
- 對 `source=payuni`、`status=paid`、**未開通** 的訂單，提供**三種開通操作，皆為明確按鈕**：
  1. **逐筆開通**：每筆未開通訂單一顆「開通」按鈕。
  2. **勾選開通**：每筆列前有核取方塊，選取多筆後按「**開通勾選（N）**」一次開通所選。
  3. **全數開通**：一顆「**全部開通（尚有 M 筆未開通）**」按鈕，一鍵開通目前**所有**未開通訂單。
- 勾選／全數開通前彈**確認對話**（「確定開通這 N 筆？」）；`grantAccess` 冪等，重複或並發開通皆安全。
- 開通 ＝ 沿用 `grantAccess`（`lib/fulfillment-grant.js`，與現有手動開通同源、冪等），開通後可寄「開通／開課」信。
- 後端：擴充 `/api/admin/grant-access`（或新增批次端點）接受**多筆訂單／email** 一次開通，回報逐筆結果（成功／已開通／失敗）。
