...
ALTER TABLE orders
  ...
  ADD COLUMN IF NOT EXISTS invoice_claimed_at TIMESTAMPTZ; -- 開票原子 claim 旗標：notify 並發／重送時只有搶到者呼叫 Amego（開票失敗清回 null 以便重試）
...
-- 新 IP 登入偵測每次登入都查 (action, ip)
CREATE INDEX IF NOT EXISTS admin_audit_log_action_ip_idx ON admin_audit_log (action, ip);
