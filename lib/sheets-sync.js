...
// 退款日期／金額：優先取 orders.refunded_at／refund_amount（supabase-payment-events.sql 新增，
// 退款當下寫入、不受 updated_at trigger 影響）。
// 這兩欄之前的舊訂單（或 SQL 尚未執行）沒有值 → 退回以 updated_at 與原金額推算；
// 注意 updated_at 會被退款後的任何操作（補寄信、開發票）蓋掉，只是不得已的近似值。
// 只有退款單才輸出退款日期：日後若後台開放改狀態，不要讓一筆 paid 單帶著退款日期進記帳表
const refundedAt = (o) => (o.status === "refunded" ? o.refunded_at || o.updated_at || "" : "");
const refundAmount = (o) => {
  if (o.refund_amount != null && o.refund_amount !== "") return Number(o.refund_amount) || 0;
  return o.status === "refunded" ? Number(o.amount) || 0 : "";
};
