...
import { isFanProofOpen, buildFanCoupon, isOwnProofUrl, FAN_PRICE, FAN_PLAN, FAN_PROOF_DEADLINE, FAN_DIRECT_PRICE, FAN_PROOF_DISCOUNT } from "./fan-proof.js";
...
describe("buildFanCoupon", () => {
  it("產生 bundle 一次性「折抵 300 元」券（跟著波段價走，非固定成交價）", () => {
...
describe("常數", () => {
  it("FAN_PRICE=3699 / FAN_PLAN=bundle", () => {
    expect(FAN_PRICE).toBe(3699);
    expect(FAN_PROOF_DISCOUNT).toBe(300);
...
describe("buildFanCoupon 帶 price 參數", () => {
  it("自訂 price 寫進 value", () => {
    expect(buildFanCoupon({ code: "FANX1234", discount: 500 })).toMatchObject({ value: 500, type: "fixed", plan: "bundle", usage_limit: 1 });
  });
  it("不帶 discount 用 FAN_PROOF_DISCOUNT 預設", () => {
    expect(buildFanCoupon({ code: "FANX1234" }).value).toBe(FAN_PROOF_DISCOUNT);
  });
});
