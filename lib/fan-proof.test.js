...
import { isFanProofOpen, buildFanCoupon, isOwnProofUrl, FAN_PRICE, FAN_PLAN, FAN_PROOF_DEADLINE } from "./fan-proof.js";

describe("isFanProofOpen", () => {
  it("截止前為 true", () => {
    expect(isFanProofOpen(new Date("2026-07-31T23:00:00+08:00"))).toBe(true);
  });
  it("截止當下邊界（23:59:59）為 true", () => {
    expect(isFanProofOpen(new Date("2026-07-31T23:59:59+08:00"))).toBe(true);
  });
  it("8/1 00:00 之後為 false", () => {
    expect(isFanProofOpen(new Date("2026-08-01T00:00:00+08:00"))).toBe(false);
  });
});
