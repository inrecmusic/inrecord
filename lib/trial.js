...
export function buildTrialUrl(email, siteUrl = "https://inrecordmusic.com") {
  ...
  // 帶 UTM：之後才追得到「試看信帶來幾筆成交」
  return `${siteUrl}/trial?e=${encodeURIComponent(e)}&t=${signTrialToken(e)}&utm_source=email&utm_medium=email&utm_campaign=trial`;
}
...
export function buildTrialEmail({ email, siteUrl = "https://inrecordmusic.com" }) {
  ...
  // 主旨會被版面當成大標印在 Logo 正下方，所以不帶「InRecord｜」前綴（會與 Logo 重複）
  const subject = "您的免費試看課程影片來了 🎹";
  const bodyMd = [
    "@badge 免費試看",
    ...
    "您好：", "",
    "感謝您留下 Email！", "",
    "為您送上《從零開始學鋼琴－了解三和弦與基礎伴奏》的免費試看課程影片，點擊下方按鈕即可開始觀看：", "",
    `[觀看免費試看課程影片](${url})`, "",
    "若您在觀看後希望能更有系統地學習，歡迎前往官網查看最新的課程方案與優惠。若有任何疑問，也歡迎回覆此信件，我們將竭誠為您解答。", "",
    "**InRecord｜音樂刻**",
  ].join("\n");
