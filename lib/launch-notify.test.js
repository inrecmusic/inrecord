...
// supabase mock：enrollments(.select.eq.range)＝名單、orders(.select.eq.or.range)＝dryRun 附加統計、launch_notify_sends(.select.range / .insert / .delete.eq)、
// sale_settings(.update.eq.is)。range 是 selectAll 分頁用的收尾呼叫。
// claimConflicts：模擬「讀完名單之後、佔位之前」被另一路（cron／後台按鈕）搶走的 email。
function makeSupabase({ paid = [], alreadySent = [], ordersError = null, claimConflicts = [], paidOrders = [] }) {
  ...
  const sb = {
    from(table) {
      if (table === "enrollments" || table === "orders") {
        const rows = ((table === "orders" ? paidOrders : paid) || []).map((e) => ({ email: e }));
        const range = (from, to) =>
          Promise.resolve(ordersError && table === "enrollments" ? { data: null, error: ordersError } : { data: page(rows, from, to), error: null });
        return { select: () => ({ eq: () => ({ range, or: () => ({ range }) }) }) };
      }
...
describe("runLaunchNotify", () => {
  ...
  it("dryRun：只算人數（含已寄過／待寄／已付款未開通），不寄、不佔位、不標記完成", async () => {
    const sent = [];
    const { sb, recorded, state } = makeSupabase({ paid: ["a@x.com", "b@x.com", "c@x.com"], alreadySent: ["a@x.com"], paidOrders: ["b@x.com", "unpaid-grant@x.com", "UNPAID-GRANT@x.com"] });
    const r = await runLaunchNotify(sb, { sendLaunchEmail: async ({ email }) => { sent.push(email); return { success: true }; }, dryRun: true });
    expect(r).toEqual({ dryRun: true, total: 3, alreadySent: 1, pending: 2, unenrolledPaid: 1 });
    expect(sent).toEqual([]);
    expect(recorded.has("b@x.com")).toBe(false);
    expect(state.completedAt).toBeNull();
  });
