...
import { createRateLimiter, createDistributedLimiter, clientIp } from "./rate-limit.js";
...
describe("createDistributedLimiter（無 Upstash env → 記憶體保底）", () => {
  beforeEach(() => {
    delete process.env.UPSTASH_REDIS_REST_URL;
    delete process.env.UPSTASH_REDIS_REST_TOKEN;
  });

  it("缺 Upstash 設定時退回記憶體限流，仍會擋超量請求", async () => {
    const limit = createDistributedLimiter({ limit: 2, windowMs: 60_000 });
    expect((await limit("ip-a")).allowed).toBe(true);
    expect((await limit("ip-a")).allowed).toBe(true);
    expect((await limit("ip-a")).allowed).toBe(false); // 第 3 次擋
  });
});
