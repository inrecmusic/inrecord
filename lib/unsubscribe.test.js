...
describe("recordUnsubscribe / excludeUnsubscribed", () => {
  // readError：模擬讀取失敗的 PostgREST 錯誤；未給則正常回 rows（支援 selectAll 的 .select().range()）
    ...
    return {
      ...
      from(table) {
        ...
        const b = {
          ...
          select: () => b,
          range: (from, to) => Promise.resolve(readError ? { data: null, error: readError } : { data: rows.slice(from, to + 1), error: null }),
        };
        return b;
      },
    };
  ...
  it("表不存在（新環境尚未跑 SQL）→ fail-open 照原名單", async () => {
    vi.spyOn(console, "error").mockImplementation(() => {});
    const db = fakeDb({ readError: { message: 'relation "newsletter_unsubscribes" does not exist' } });
    expect(await excludeUnsubscribed(db, ["a@x.com"])).toEqual(["a@x.com"]);
    const db2 = fakeDb({ readError: { code: "PGRST205", message: "Could not find the table in the schema cache" } });
    expect(await excludeUnsubscribed(db2, ["a@x.com"])).toEqual(["a@x.com"]);
  });
  it("其他讀取失敗（DB 抖動）→ fail-closed 丟錯中止群發，不可把信寄給已退訂者", async () => {
    vi.spyOn(console, "error").mockImplementation(() => {});
    const db = fakeDb({ readError: { code: "57014", message: "canceling statement due to statement timeout" } });
    await expect(excludeUnsubscribed(db, ["a@x.com"])).rejects.toThrow(/unsubscribe_list_unavailable/);
  });
  it("超過 1000 筆退訂者也要全部排除（分頁撈取，不被 PostgREST 上限截斷）", async () => {
    const rows = Array.from({ length: 1200 }, (_, i) => ({ email: `u${i}@x.com` }));
    const db = fakeDb({ rows });
    expect(await excludeUnsubscribed(db, ["u1199@x.com", "keep@x.com"])).toEqual(["keep@x.com"]);
  });
