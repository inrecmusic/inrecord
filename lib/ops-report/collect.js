...
export function buildPack({ period, now = new Date(), orders = [], prevOrders = [], enrolledEmails = [], progressRows = [], emailLog = [], unsubscribes = [], coupons = [], saleSettings = {}, videos = [], announcements = [], adReport = null, brevoQuota = null, leadsCount = null }) {
  ...
  const missing = published.filter((v) => !v.bunny_video_id);
  return {
    ...
    students: {
      ...
      active_this_week: new Set(weekProgress.map((p) => p.user_id)).size,
    ...
    content: { published_units: published.length, units_without_video: missing.length, first_missing_unit: missing[0]?.title || null },
