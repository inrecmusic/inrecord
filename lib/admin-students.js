...
export function mergeStudents({ enrollments = [], orders = [], leads = [], profiles = [] } = {}) {
  ...
  // email → 真的付過錢（排除 source='manual' 的手動開通單，那是測試／贈送，金額 0）
  const realPaidByEmail = new Set();
  for (const o of orders) {
    const k = norm(o.email);
    if (!k) continue;
    if (o.source !== "manual") realPaidByEmail.add(k);
  ...
  for (const k of emails) {
    ...
    const enrolled = !!enr;
    const hasAnyOrder = !!ord;                    // 有任何 status='paid' 的訂單（含手動開通）→ 是學員
    const hasPaidOrder = realPaidByEmail.has(k);  // 真的付過錢（手動開通單不算）→ paid 旗標
    out.push({
      ...
      purchased: enrolled || hasAnyOrder || lead?.status === "purchased",
      // paid = 確實有 status='paid' 且非手動開通（source≠'manual'）的訂單。purchased 還含「只開通沒訂單」（測試帳號）
      // 與「人工標記已購買」，後台要只看真實付費學員時用這個旗標。
      paid: hasPaidOrder,
      status: (enrolled || hasAnyOrder) ? "purchased" : (lead?.status || "requested"),
...
// 後台手動開通單（source='manual'、amount 0）沒有付款，不算購買者（2026-09-04 使用者定案：測試／贈送帳號不灌水）。
