...
// 解析 Amego 回傳（純函式）。成功必須 code===0 且有 invoice_number；
// ⚠️ 發票號碼一律取 invoice_number，不可 fallback 用 msg（成功時 msg 為空，誤用會污染發票號）。
export function parseAmegoResult(result) {
  const codeOk = result?.code === "0" || result?.code === 0;
  if (codeOk && result.invoice_number) {
    return {
      success: true,
...
export async function createInvoice({
  ...
  const data = {
    ...
    ProductItem: [
      {
        Description: productName || "從零開始學鋼琴",
        Quantity: "1",
  ...
  try {
    ...
    const parsed = parseAmegoResult(result);
    if (!parsed.success) console.error("[Amego] 開立失敗 code:", result?.code, result?.msg);
    return parsed;
  } catch (err) {
