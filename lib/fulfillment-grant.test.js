...
// 精簡 supabase 替身：記錄 upsert/insert 呼叫（驗分流邏輯，非測 mock 行為）
function fakeSupabase({ errors = {} } = {}) {
  const calls = [];
  const sb = {
    from(table) {
      const rec = (method) => (row, opts) => {
        calls.push({ table, method, row, opts });
        return Promise.resolve({ error: errors[table] || null });
      };
      return { upsert: rec("upsert"), insert: rec("insert") };
    },
  };
  return { sb, calls };
}
...
describe("grantAccess", () => {
  ...
  it("bundle 方案：開通 enrollments(upsert) 與 subscriptions(insert, plan_type=bundle, 永久)", async () => {
    const { sb, calls } = fakeSupabase();
    ...
    // 改用 insert（partial unique index 與 supabase-js onConflict 不相容）
    expect(sub.method).toBe("insert");
    expect(sub.opts).toBeUndefined();
  ...
  it("game 方案：只開通 subscriptions(insert, plan_type=game)，不碰 enrollments", async () => {
    const { sb, calls } = fakeSupabase();
  ...
  it("enrollments 失敗時 ok=false 並帶錯誤訊息", async () => {
  ...
  it("subscriptions 撞唯一索引(23505) 視為冪等成功，不算錯誤", async () => {
    const { sb } = fakeSupabase({ errors: { subscriptions: { code: "23505", message: "duplicate key" } } });
    const res = await grantAccess(sb, { id: "o5", email: "a@b.com", plan: "bundle" });
    expect(res.ok).toBe(true); // enrollments OK + subscriptions 23505 容忍
  });

  it("subscriptions 非 23505 錯誤仍回報 ok=false", async () => {
    const { sb } = fakeSupabase({ errors: { subscriptions: { code: "42P01", message: "no table" } } });
    const res = await grantAccess(sb, { id: "o6", email: "a@b.com", plan: "game" });
    expect(res.ok).toBe(false);
    expect(res.errors.join(" ")).toMatch(/no table/);
  });
});
