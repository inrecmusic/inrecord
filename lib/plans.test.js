...
import { PLAN_CATALOG, applyCoupon, couponError, couponPlanError } from "./plans.js";

describe("PLAN_CATALOG（後端權威價格）", () => {
  it("兩方案的 key、價格、品名與規格一致", () => {
    expect(PLAN_CATALOG.course).toEqual({ price: 3800, label: "鋼琴自學全課程", sellable: false });
    expect(PLAN_CATALOG.bundle).toEqual({ price: 13800, label: "完整課程方案" });
  });
...
describe("couponError", () => {
  ...
  it("起訖以台灣日期為準，不受伺服器時區影響（Vercel 是 UTC）", () => {
    // ends_at 6/30：台灣 6/30 23:30 仍有效、7/1 00:30 已過期（舊寫法在 UTC 伺服器要到 7/1 08:00 才過期）
    expect(couponError(valid, new Date("2026-06-30T23:30:00+08:00"))).toBeNull();
    expect(couponError(valid, new Date("2026-07-01T00:30:00+08:00"))).toBe("coupon_expired");
    // starts_at 6/1：台灣 6/1 00:30 已可用（舊寫法要到 6/1 08:00）
    expect(couponError(valid, new Date("2026-06-01T00:30:00+08:00"))).toBeNull();
    expect(couponError(valid, new Date("2026-05-31T23:30:00+08:00"))).toBe("coupon_not_started");
  });
