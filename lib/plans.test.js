import { describe, it, expect } from "vitest";
import {
  PLAN_CATALOG, applyCoupon, couponError, couponPlanError,
  normalizeCouponType, normalizeCouponPlan, couponValueError,
} from "./plans.js";

describe("PLAN_CATALOG（後端權威價格）", () => {
  it("兩方案的 key、價格、品名與規格一致", () => {
    expect(PLAN_CATALOG.course).toEqual({ price: 3800, label: "鋼琴自學全課程" });
    expect(PLAN_CATALOG.bundle).toEqual({ price: 3999, label: "學琴全攻略" });
  });

  it("不存在未預期的方案 key（AI 練功房 game 已下架）", () => {
    expect(Object.keys(PLAN_CATALOG).sort()).toEqual(["bundle", "course"]);
  });
});

describe("applyCoupon", () => {
  it("無優惠券時回傳原價", () => {
    expect(applyCoupon(3800, null)).toBe(3800);
    expect(applyCoupon(3800, undefined)).toBe(3800);
  });

  it("percent 折扣四捨五入", () => {
    // 3999 * (1 - 0.1) = 3599.1 → 3599
    expect(applyCoupon(3999, { type: "percent", value: 10 })).toBe(3599);
    // 3800 * 0.85 = 3230
    expect(applyCoupon(3800, { type: "percent", value: 15 })).toBe(3230);
    // 1200 * 0.50 = 600
    expect(applyCoupon(1200, { type: "percent", value: 50 })).toBe(600);
  });

  it("percent 100% 折到 0", () => {
    expect(applyCoupon(3800, { type: "percent", value: 100 })).toBe(0);
  });

  it("fixed 折抵固定金額", () => {
    expect(applyCoupon(3800, { type: "fixed", value: 500 })).toBe(3300);
    expect(applyCoupon(1200, { type: "fixed", value: 200 })).toBe(1000);
  });

  it("折後價不會低於 0", () => {
    expect(applyCoupon(1200, { type: "fixed", value: 5000 })).toBe(0);
    expect(applyCoupon(1200, { type: "percent", value: 200 })).toBe(0);
  });
});

describe("couponError", () => {
  const now = new Date("2026-06-08T12:00:00+08:00");
  const valid = {
    code: "SAVE10",
    type: "percent",
    value: 10,
    status: "active",
    starts_at: "2026-06-01",
    ends_at: "2026-06-30",
    used: 0,
    usage_limit: 100,
  };

  it("有效優惠券回傳 null", () => {
    expect(couponError(valid, now)).toBeNull();
  });

  it("找不到優惠券", () => {
    expect(couponError(null, now)).toBe("coupon_not_found");
    expect(couponError(undefined, now)).toBe("coupon_not_found");
  });

  it("停用狀態", () => {
    expect(couponError({ ...valid, status: "disabled" }, now)).toBe("coupon_inactive");
  });

  it("尚未開始", () => {
    expect(couponError({ ...valid, starts_at: "2026-07-01" }, now)).toBe("coupon_not_started");
  });

  it("已過期（ends_at 早於今天）", () => {
    expect(couponError({ ...valid, ends_at: "2026-06-07" }, now)).toBe("coupon_expired");
  });

  it("ends_at 當天仍有效（包含整天到 23:59:59）", () => {
    expect(couponError({ ...valid, ends_at: "2026-06-08" }, now)).toBeNull();
  });

  it("用量達上限", () => {
    expect(couponError({ ...valid, used: 100, usage_limit: 100 }, now)).toBe("coupon_used_up");
    expect(couponError({ ...valid, used: 150, usage_limit: 100 }, now)).toBe("coupon_used_up");
  });

  it("usage_limit 為 null 代表無限制", () => {
    expect(couponError({ ...valid, used: 9999, usage_limit: null }, now)).toBeNull();
  });

  it("無 starts_at / ends_at 時不檢查時間", () => {
    expect(couponError({ ...valid, starts_at: null, ends_at: null }, now)).toBeNull();
  });
});

describe("applyCoupon price 型（指定成交價）", () => {
  it("回指定價（低於基準）", () => expect(applyCoupon(5800, { type: "price", value: 2500 })).toBe(2500));
  it("clamp：指定價高於基準 → 取基準", () => expect(applyCoupon(2000, { type: "price", value: 2500 })).toBe(2000));
  it("邊界：指定價等於基準 → 取基準", () => expect(applyCoupon(2500, { type: "price", value: 2500 })).toBe(2500));
  it("不低於 0", () => expect(applyCoupon(5800, { type: "price", value: -100 })).toBe(0));
});

describe("couponPlanError 方案鎖", () => {
  it("方案相符 → null", () => expect(couponPlanError({ plan: "bundle" }, "bundle")).toBeNull());
  it("方案不符 → coupon_wrong_plan", () => expect(couponPlanError({ plan: "bundle" }, "course")).toBe("coupon_wrong_plan"));
  it("coupon.plan 為 null（不限）→ null", () => expect(couponPlanError({ plan: null }, "course")).toBeNull());
  it("無 coupon → null（交給 couponError 處理）", () => expect(couponPlanError(null, "bundle")).toBeNull());
});

describe("normalizeCouponType（admin 寫入正規化）", () => {
  it("合法值原樣保留", () => {
    expect(normalizeCouponType("percent")).toBe("percent");
    expect(normalizeCouponType("fixed")).toBe("fixed");
    expect(normalizeCouponType("price")).toBe("price");
  });
  it("非法/缺值 → percent", () => {
    expect(normalizeCouponType("garbage")).toBe("percent");
    expect(normalizeCouponType(undefined)).toBe("percent");
    expect(normalizeCouponType(null)).toBe("percent");
  });
});

describe("normalizeCouponPlan（admin 寫入正規化）", () => {
  it("PLAN_CATALOG 既有方案保留", () => {
    expect(normalizeCouponPlan("course")).toBe("course");
    expect(normalizeCouponPlan("bundle")).toBe("bundle");
  });
  it("非法/缺值/繼承屬性 → null（不限方案）", () => {
    expect(normalizeCouponPlan("game")).toBeNull();        // 已下架方案
    expect(normalizeCouponPlan("garbage")).toBeNull();
    expect(normalizeCouponPlan("")).toBeNull();
    expect(normalizeCouponPlan(undefined)).toBeNull();
    expect(normalizeCouponPlan(null)).toBeNull();
    expect(normalizeCouponPlan("toString")).toBeNull();    // 不可命中原型鏈
  });
});

describe("couponValueError（admin 寫入驗證）", () => {
  it("合法值 → null", () => {
    expect(couponValueError("percent", 50)).toBeNull();
    expect(couponValueError("percent", 100)).toBeNull();
    expect(couponValueError("fixed", 500)).toBeNull();
    expect(couponValueError("price", 5000)).toBeNull();    // price 無上限（applyCoupon 夾擠）
  });
  it("非數值/<=0 → invalid_value", () => {
    expect(couponValueError("fixed", 0)).toBe("invalid_value");
    expect(couponValueError("fixed", -100)).toBe("invalid_value");
    expect(couponValueError("price", NaN)).toBe("invalid_value");
    expect(couponValueError("percent", undefined)).toBe("invalid_value");
  });
  it("percent > 100 → percent_over_100", () => {
    expect(couponValueError("percent", 150)).toBe("percent_over_100");
    // fixed/price 不受 100 上限限制
    expect(couponValueError("fixed", 150)).toBeNull();
  });
});
