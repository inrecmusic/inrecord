...
describe("hasTag", () => {
  it("不再用日期猜有沒有 tag（寫程式日期 ≠ 部署日期），一律回 false 由 summarizeGroup 自動偵測", () => {
    expect(hasTag({ dateTW: "2026-09-20" })).toBe(false);
    expect(hasTag({ dateTW: "2026-08-01" })).toBe(false);
  });
});
...
describe("事件歸屬（迴歸）", () => {
  ...
  it("B2：事件裡出現過自己的 tag 就轉精準模式，沒帶 tag 的事件不算", () => {
    const rows = [row("a@x.com", "九月報", "newsletter", "2026-09-20T02:00:00Z")];
    const g = groupSends(rows)[0];
    const evs = [
      ev("a@x.com", "delivered", "2026-09-20T02:05:00Z", { tag: "newsletter" }),
      ev("a@x.com", "opened", "2026-09-20T03:00:00Z"), // 沒帶 tag → 精準模式下不採計
    ];
    // 自動偵測：有一筆帶了 newsletter tag → 精準模式，沒帶 tag 的 opened 不算
    expect(summarizeGroup(g, evs)).toMatchObject({ precise: true, delivered: 1, opened: 0 });
    expect(summarizeGroup(g, evs, { requireTag: false })).toMatchObject({ precise: false, delivered: 1, opened: 1 });
  });
