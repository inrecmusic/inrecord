...
describe("selectRecoveryCandidates", () => {
  ...
  it("近 7 天已寄過的 email（不分大小寫）跳過；同一批同 email 只留第一筆", () => {
    const r = selectRecoveryCandidates([
      { ...base, id: 1, created_at: iso(7), email: "Spam@X.com" },   // 7 天內寄過 → 跳過
      { ...base, id: 2, created_at: iso(8), email: "b@x.com" },
      { ...base, id: 3, created_at: iso(9), email: "B@x.com" },      // 同批同 email → 只留 id 2
      { ...base, id: 4, created_at: iso(10), email: "c@x.com" },
    ], now, { recentEmails: ["spam@x.com"] });
...
describe("buildRecoveryEmail", () => {
  it("含方案名與帶 UTM 的 CTA", () => {
    ...
    expect(subject).toContain("尚未完成付款");
    expect(html).toContain("學琴全攻略");
    expect(html).toContain("utm_campaign=abandoned_recovery");
    expect(html).toContain("mascot-grand-v1.png");          // 吉祥物
    expect(html).toContain("logo-wordmark-white.png");      // 品牌頁首
    expect(html).toContain("完成付款");
    expect(html).toContain("上架時程");            // 與其他信件一致的時間軸
    expect(html).not.toContain("名額有限");
  });
