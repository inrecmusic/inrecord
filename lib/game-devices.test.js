...
import { pickAllowedDeviceIds, buildWatermark, exceedsDeviceLimit, devicesToEvict } from "./game-devices";
...
describe("devicesToEvict", () => {
  const d = (id, t) => ({ device_id: id, last_seen_at: t });

  it("裝置數 ≤ limit：不擠（空陣列）", () => {
    const out = devicesToEvict([d("a", "2026-08-01"), d("b", "2026-08-02")], 3);
    expect(out).toEqual([]);
  });

  it("裝置數 > limit：回超出的最舊那些（依 last_seen_at，最舊排最前）", () => {
    const out = devicesToEvict(
      [d("old", "2026-08-01"), d("mid", "2026-08-05"), d("new", "2026-08-10"), d("older", "2026-07-01")],
      2
    );
    // 排序後（新→舊）：new, mid, old, older；留 2 台(new, mid) → 擠出 old, older
    expect(out).toEqual(["old", "older"]);
  });

  it("空陣列 → 空", () => {
    expect(devicesToEvict([], 3)).toEqual([]);
  });

  it("limit=0 邊界：全部擠掉", () => {
    const out = devicesToEvict([d("a", "2026-08-01"), d("b", "2026-08-02")], 0);
    expect(out.sort()).toEqual(["a", "b"]);
  });
});
