...
export function buildAdDailyEmail({ report, dayLabel, targetRoas = 3, siteUrl = "https://inrecordmusic.com" } = {}) {
  ...
  // 冷啟動階段（成交太少）多印名單數與名單成本——這階段該看的是這兩個，不是 ROAS
  const cold = advice?.cold_start === true;
  const rows = (report.campaigns || []).filter((c) => (Number(c.spend) || 0) > 0)
    .map((c) => {
      const base = `- **${c.campaign_name || c.campaign_id}**：花 ${nt(c.spend)}、${c.orders} 筆成交、收 ${nt(c.revenue)}、ROAS ${r2(c.trueRoas)}`;
      if (!cold) return base;
      const leads = Number(c.metaConversions) || 0;
      const perLead = leads ? nt((Number(c.spend) || 0) / leads) : "—";
      return `${base}；名單 ${leads} 個、每個 ${perLead}`;
    });
  ...
  const bodyMd = [
    ...
    ...(cold && advice?.cold_start_note ? ["## 目前用什麼標準判斷", advice.cold_start_note, ""] : []),
