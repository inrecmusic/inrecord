...
describe("mdToHtml 連結與按鈕", () => {
  ...
  it("連結前後有文字則不是按鈕、是行內連結", () => {
    ...
    expect(h).not.toContain("text-align:center");
    expect(h).toContain('style="color:#2563eb;"');
  });
  it("連結文字內的粗體仍生效", () => {
    expect(mdToHtml("[**立即**報名](https://x.com/a)")).toContain("<strong>立即</strong>報名");
  });
});

describe("品牌外框（Logo header）", () => {
  it("電子報與自訂信都含 logo 圖與品牌列", () => {
    const nl = renderNewsletterHtml({ subject: "x", bodyMd: "hi" });
    const ad = renderAdminEmailHtml({ subject: "y", bodyMd: "hi" });
    for (const h of [nl, ad]) {
      expect(h).toContain('src="https://inrecordmusic.com/logo-wordmark.png"');
      expect(h).toContain('alt="InRecord"');
  ...
  it("siteUrl 覆寫時 logo 與連結跟著換 host", () => {
    const h = renderNewsletterHtml({ subject: "x", bodyMd: "hi", siteUrl: "https://preview.example" });
    expect(h).toContain('src="https://preview.example/logo-wordmark.png"');
  });
});
