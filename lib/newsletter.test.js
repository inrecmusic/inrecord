...
import { mdToHtml, renderNewsletterHtml, renderAdminEmailHtml, dedupeEmails, tagNewsletterLinks } from "./newsletter.js";

describe("段落內換行（連續行併成同一段）", () => {
  it("連續兩行 → 同一個 <p>，中間 <br>；不是各自成段", () => {
    const html = mdToHtml("第一行。\n第二行。");
    expect(html.match(/<p /g) || []).toHaveLength(1);
    expect(html).toContain("第一行。<br>第二行。");
  });

  it("空一行 → 真的分成兩段（段距由 <p> 的 margin 提供）", () => {
    const html = mdToHtml("第一段。\n\n第二段。");
    expect(html.match(/<p /g) || []).toHaveLength(2);
    expect(html).not.toContain("<br>");
  });

  it("標題與清單會把前面累積的內文收掉，不會被吸進同一段", () => {
    const html = mdToHtml("前言。\n## 標題\n內文。\n- 項目\n收尾。");
    expect(html.indexOf("前言。")).toBeLessThan(html.indexOf("<h2"));
    expect(html).toContain(">內文。</p>");
    expect(html).toContain(">收尾。</p>");
    expect(html).not.toContain("內文。<br>");
  });

  it("行內樣式仍逐行套用（粗體、連結不會因為合併而失效）", () => {
    const html = mdToHtml("**粗體行**\n一般行有[連結](https://inrecordmusic.com/x)。");
    expect(html).toContain("<strong");
    expect(html).toContain('href="https://inrecordmusic.com/x"');
    expect(html.match(/<p /g) || []).toHaveLength(1);
  });
});
...
describe("mdToHtml", () => {
  ...
  it("連續 1. 2. 行併成 ol；空行結束清單；plain 模式出 <ol>，email 模式出編號 div", () => {
    expect(mdToHtml("1. 甲\n2. 乙\n\n段落", { plain: true })).toBe("<ol><li>甲</li><li>乙</li></ol><p>段落</p>");
    const html = mdToHtml("1. 甲\n2. 乙");
    expect(html).toContain(">1.</span>甲");
    expect(html).toContain(">2.</span>乙");
    expect(html).not.toContain("<ol>");
  });
  it("- 與 1. 相鄰時各自成一組清單、不混在一起", () => {
    expect(mdToHtml("- a\n1. b", { plain: true })).toBe("<ul><li>a</li></ul><ol><li>b</li></ol>");
  });
  it("--- 變分隔線", () => {
...
describe("時間軸卡片 :::timeline", () => {
  it("區塊轉成藍圈卡片，dim 列灰階、次文字顯示", () => {
    ...
    expect(h).toContain("晚上\u00a08:00");   // 中文與數字之間改用不斷行空格，避免「晚上」與「8:00」被拆到兩行                        // 次文字
...
describe("品牌外框（Logo header）", () => {
  it("電子報與自訂信都有深色頁首＋白字 Logo＋標題反白＋品牌列", () => {
    ...
    for (const h of [nl, ad]) {
      ...
      expect(h).toContain("InRecord 音樂刻");                                    // 品牌簽名檔
    }
...
describe("renderNewsletterHtml 退訂按鈕", () => {
  it("有 unsubscribeUrl → 出「取消訂閱」按鈕連結、不再出回信告知句", () => {
    const html = renderNewsletterHtml({ subject: "S", bodyMd: "hi", siteUrl: "https://inrecordmusic.com", unsubscribeUrl: "https://inrecordmusic.com/unsubscribe?e=a%40x.com&t=abc" });
    expect(html).toContain('href="https://inrecordmusic.com/unsubscribe?e=a%40x.com&amp;t=abc"');
    expect(html).toContain(">取消訂閱</a>");
    expect(html).toContain("登入驗證碼、購課與開通通知不受影響");
    expect(html).not.toContain("直接回信告知");
  });
  it("無 unsubscribeUrl → 舊的回信告知句（相容）", () => {
    const html = renderNewsletterHtml({ subject: "S", bodyMd: "hi", siteUrl: "https://inrecordmusic.com" });
    expect(html).toContain("直接回信告知");
    expect(html).not.toContain(">取消訂閱</a>");
  });
});

describe("mdToHtml 行內連結 URL 含 *（不可被當成強調標記）", () => {
  it("單一連結 URL 含 * 保持完整", () => {
    const html = mdToHtml("看這裡 [活動頁](https://x.com/a*b/c*d) 謝謝");
    expect(html).toContain('<a href="https://x.com/a*b/c*d" style="color:#2563eb;">活動頁</a>');
    expect(html).not.toContain("<em>");
  });
  it("兩個連結之間的 * 不會把 href 串起來", () => {
    const html = mdToHtml("[A](https://x.com/*p) 與 [B](https://y.com/q*)");
    expect(html).toContain('href="https://x.com/*p"');
    expect(html).toContain('href="https://y.com/q*"');
  });
  it("連結文字內的粗體仍有效", () => {
    const html = mdToHtml("請點 [**重要**](https://x.com/z) 查看");
    expect(html).toContain('<a href="https://x.com/z" style="color:#2563eb;"><strong style="color:#0f172a;">重要</strong></a>');
  });
});

describe("品牌簽名檔", () => {
  it("電子報與後台單封信都含完整簽名（slogan／官網／Email／IG）", () => {
    for (const html of [
      renderNewsletterHtml({ subject: "S", bodyMd: "hi", siteUrl: "https://inrecordmusic.com" }),
      renderAdminEmailHtml({ subject: "S", bodyMd: "hi", siteUrl: "https://inrecordmusic.com" }),
    ]) {
      expect(html).toContain("InRecord 音樂刻");
      expect(html).toContain("玩轉音樂的每一課");
      expect(html).toContain("mailto:support@inrecordmusic.com");
      expect(html).toContain("https://www.instagram.com/inrecord.music");
    }
  });
});

// 圖片對齊：照片型的圖靠左與內文對齊，吉祥物等裝飾圖維持置中
describe("圖片 |left 對齊", () => {
  it("不帶 left → 置中", () => {
    expect(mdToHtml("![a](https://x/p.jpg|520)")).toContain("text-align:center");
  });
  it("帶 |left → 靠左，寬度照樣吃前一段", () => {
    const html = mdToHtml("![a](https://x/p.jpg|520|left)");
    expect(html).toContain("text-align:left");
    expect(html).toContain('width="520"');
  });
});

// 群發時自動補 UTM：靠人工在每封信每個連結手動加，遲早會漏
describe("tagNewsletterLinks", () => {
  const site = "https://inrecordmusic.com";
  it("自家連結補上 campaign＝草稿代號，錨點留在最後", () => {
    const out = tagNewsletterLinks("[買](https://inrecordmusic.com/#pricing)", site, "lastcall");
    expect(out).toBe("[買](https://inrecordmusic.com/?utm_source=newsletter&utm_medium=email&utm_campaign=lastcall#pricing)");
  });
  it("已有查詢字串用 & 接", () => {
    expect(tagNewsletterLinks("[試看](https://inrecordmusic.com/trial?e=a)", site, "guide"))
      .toContain("/trial?e=a&utm_source=newsletter");
  });
  it("已指定 utm_source 的不覆寫；外部連結、圖片不動", () => {
    const keep = "[x](https://inrecordmusic.com/?utm_source=ig)";
    expect(tagNewsletterLinks(keep, site, "c")).toBe(keep);
    const out = "[y](https://example.com/a)";
    expect(tagNewsletterLinks(out, site, "c")).toBe(out);
    const img = "![圖](https://inrecordmusic.com/a.png)";
    expect(tagNewsletterLinks(img, site, "c")).toBe(img);
  });
  it("沒有 campaign 或不是字串 → 原樣回傳", () => {
    expect(tagNewsletterLinks("[a](https://inrecordmusic.com/)", site, "")).toBe("[a](https://inrecordmusic.com/)");
    expect(tagNewsletterLinks(null, site, "c")).toBe(null);
  });
});
