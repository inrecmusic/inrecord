...
describe("pickStrip", () => {
  ...
  it("提示條放「最新的一則未讀」，不受置頂影響", () => {
    expect(pickStrip(sorted, { read: [] }, null).id).toBe("newest");
  });

  it("讀過的不算；最新那則被關掉就不顯示", () => {
    expect(pickStrip(sorted, { read: ["older"] }, null).id).toBe("newest");
    expect(pickStrip(sorted, { read: [] }, "newest")).toBe(null);
    expect(pickStrip(sorted, { read: ["newest"] }, null).id).toBe("older");
    expect(pickStrip(sorted, { read: ["newest", "older", "pinned_old"] }, null)).toBe(null);
  });
});
