...
describe("signBunnyEmbedUrl", () => {
  ...
  it("無金鑰：回傳未簽名 URL（不含 token/expires）", () => {
    ...
    expect(url).toBe(`https://iframe.mediadelivery.net/embed/${LIB}/${VID}?autoplay=false&loop=false&muted=false&preload=true&captions=CT`);
  ...
  it("附加預設播放參數", () => {
    ...
    expect(url).toContain("captions=CT"); // 字幕預設開啟（Bunny embed 參數）
  });
});
