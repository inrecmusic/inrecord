import { describe, it, expect } from "vitest";
import { isClassroomOpen, isPresale, activeWave, listPrice, listAnchor, currentPrice, saleState, isOnSale, salePhase, fanPlanCoupon, fanCouponActive, FAN_COUPON_CODE, purchasePhaseLabel, fanProofActive, priceEndsAt } from "./sale.js";

const iso = (s) => new Date(s).toISOString();
const settings = {
  open_at: iso("2026-09-04T00:00:00+08:00"),
  lock_override: null,
  launch_notified_at: null,
  list_price: { course: 6800, bundle: 5800 },
  waves: [
    { starts_at: iso("2026-07-08T00:00:00+08:00"), ends_at: iso("2026-07-20T00:00:00+08:00"), prices: { course: 5500, bundle: 4299 } },
    { starts_at: iso("2026-07-20T00:00:00+08:00"), ends_at: iso("2026-08-06T00:00:00+08:00"), prices: { course: 5800, bundle: 4799 } },
    { starts_at: iso("2026-08-06T00:00:00+08:00"), ends_at: iso("2026-09-04T00:00:00+08:00"), prices: { course: 6200, bundle: 5299 } },
  ],
};
const tPre  = new Date("2026-07-01T00:00:00+08:00");
const tW1   = new Date("2026-07-10T00:00:00+08:00");
const tW2   = new Date("2026-07-25T00:00:00+08:00");
const tW3   = new Date("2026-08-10T00:00:00+08:00");
const tList = new Date("2026-09-10T00:00:00+08:00");

describe("activeWave / currentPrice", () => {
  it("開賣前無波段", () => expect(activeWave(settings, tPre)).toBeNull());
  it("第一波命中", () => expect(activeWave(settings, tW1)?.prices.bundle).toBe(4299));
  it("第一波 bundle", () => expect(currentPrice("bundle", settings, tW1)).toBe(4299));
  it("第二波 course", () => expect(currentPrice("course", settings, tW2)).toBe(5800));
  it("第三波 bundle", () => expect(currentPrice("bundle", settings, tW3)).toBe(5299));
  it("牌價後 bundle", () => expect(currentPrice("bundle", settings, tList)).toBe(5800));
  it("開賣前回牌價（onSale 另擋）", () => expect(currentPrice("bundle", settings, tPre)).toBe(5800));
  it("起含 now==starts_at → 該波", () => expect(currentPrice("bundle", settings, new Date("2026-07-08T00:00:00+08:00"))).toBe(4299));
  it("迄不含 now==ends_at → 下一波", () => expect(currentPrice("bundle", settings, new Date("2026-07-20T00:00:00+08:00"))).toBe(4799));
  it("波段價>牌價 → 取牌價防呆", () =>
    expect(currentPrice("bundle", { ...settings, waves: [{ starts_at: settings.waves[0].starts_at, ends_at: settings.waves[0].ends_at, prices: { bundle: 9999 } }] }, tW1)).toBe(5800));
  it("無 waves → 牌價", () => expect(currentPrice("bundle", { ...settings, waves: [] }, tW1)).toBe(5800));
  it("settings null → PLAN_CATALOG 保底價（寧可貴不可賤賣）", () => expect(currentPrice("bundle", null, tW1)).toBe(13800));
  it("list_price 缺 → PLAN_CATALOG", () => expect(currentPrice("course", { ...settings, list_price: {} }, tList)).toBe(3800));
  it("listPrice 直接取牌價", () => expect(listPrice("course", settings)).toBe(6800));
  it("listPrice 缺設定 → PLAN_CATALOG", () => expect(listPrice("course", null)).toBe(3800));
});

describe("priceEndsAt（這筆價格真正結束的時刻）", () => {
  const ms = (t) => Date.parse(t);
  const sameW1W2 = { ...settings, waves: settings.waves.map((w, i) => (i === 1 ? { ...w, prices: { ...w.prices, bundle: 4299 } } : w)) };
  it("第一波中：下一波價格不同 → 第一波結束就是截止", () =>
    expect(priceEndsAt("bundle", settings, tW1)).toBe(ms("2026-07-20T00:00:00+08:00")));
  it("最後一波中：之後是牌價（不同價）→ 最後一波結束", () =>
    expect(priceEndsAt("bundle", settings, tW3)).toBe(ms("2026-09-04T00:00:00+08:00")));
  it("牌價期間（不在波段內）→ null", () => expect(priceEndsAt("bundle", settings, tList)).toBeNull());
  it("開賣前（不在波段內）→ null", () => expect(priceEndsAt("bundle", settings, tPre)).toBeNull());
  it("下一波同價相接 → 跳過，用再下一波的結束", () =>
    expect(priceEndsAt("bundle", sameW1W2, tW1)).toBe(ms("2026-08-06T00:00:00+08:00")));
  it("一路同價到牌價 → null", () => {
    const flat = { ...settings, list_price: { ...settings.list_price, bundle: 4299 }, waves: settings.waves.map((w) => ({ ...w, prices: { ...w.prices, bundle: 4299 } })) };
    expect(priceEndsAt("bundle", flat, tW1)).toBeNull();
  });
  it("指定價券：成交價＝min(券價, 基準價) 不隨波段變 → null", () =>
    expect(priceEndsAt("bundle", settings, tW1, (p) => Math.min(2500, p))).toBeNull());
  it("百分比券：成交價隨波段變 → 第一波結束", () =>
    expect(priceEndsAt("bundle", settings, tW1, (p) => Math.round(p * 0.9))).toBe(ms("2026-07-20T00:00:00+08:00")));
  it("超過 100 個同價相接波段 → 回第 100 個的迄（提早截止，不會 fail-open 成 null）", () => {
    const t0 = Date.parse("2026-07-08T00:00:00+08:00"), H = 3_600_000;
    const many = Array.from({ length: 120 }, (_, i) => ({ starts_at: iso(new Date(t0 + i * H)), ends_at: iso(new Date(t0 + (i + 1) * H)), prices: { bundle: 4299 } }));
    expect(priceEndsAt("bundle", { ...settings, waves: many }, new Date(t0 + 30 * 60_000))).toBe(t0 + 100 * H);
  });
  it("course 方案獨立判斷（第二波 course 5800＝牌價 6800？不同 → 第二波結束）", () =>
    expect(priceEndsAt("course", settings, tW2)).toBe(ms("2026-08-06T00:00:00+08:00")));
});

describe("saleState / isOnSale", () => {
  it("開賣前 pre_launch", () => expect(saleState(settings, tPre)).toBe("pre_launch"));
  it("波段中 wave", () => expect(saleState(settings, tW2)).toBe("wave"));
  it("牌價 list", () => expect(saleState(settings, tList)).toBe("list"));
  it("無 waves → list（非 pre_launch）", () => expect(saleState({ ...settings, waves: [] }, tPre)).toBe("list"));
  it("isOnSale 開賣前 false", () => expect(isOnSale(settings, tPre)).toBe(false));
  it("isOnSale 波段中 true", () => expect(isOnSale(settings, tW1)).toBe(true));
  it("波段間隙 → list 態", () => {
    const gapped = { ...settings, waves: [
      { starts_at: new Date("2026-07-08T00:00:00+08:00").toISOString(), ends_at: new Date("2026-07-12T00:00:00+08:00").toISOString(), prices: { course: 5500, bundle: 4299 } },
      { starts_at: new Date("2026-07-20T00:00:00+08:00").toISOString(), ends_at: new Date("2026-08-06T00:00:00+08:00").toISOString(), prices: { course: 5800, bundle: 4799 } },
    ]};
    const inGap = new Date("2026-07-15T00:00:00+08:00");
    expect(saleState(gapped, inGap)).toBe("list");
    expect(currentPrice("bundle", gapped, inGap)).toBe(5800);
  });
});

describe("salePhase", () => {
  it("開賣前", () => {
    const p = salePhase(settings, tPre);
    expect(p.state).toBe("pre_launch");
    expect(p.onSale).toBe(false);
    expect(p.salesStartAt).toBe(settings.waves[0].starts_at);
    expect(p.classroomOpen).toBe(false);
  });
  it("波段中：早鳥價＋刪除線錨點＋nextIncreaseAt", () => {
    const p = salePhase(settings, tW1);
    expect(p.state).toBe("wave");
    expect(p.plans.bundle).toEqual({ price: 4299, originalPrice: 5800, isEarlyBird: true });
    expect(p.nextIncreaseAt).toBe(settings.waves[0].ends_at);
  });
  it("牌價＋開課", () => {
    const p = salePhase(settings, tList);
    expect(p.plans.course).toEqual({ price: 6800, originalPrice: 6800, isEarlyBird: false });
    expect(p.classroomOpen).toBe(true);
    expect(p.nextIncreaseAt).toBeNull();
  });
});

describe("listAnchor / 劃線原價≠正式售價", () => {
  const anchored = {
    ...settings,
    list_price: { bundle: 7999 },     // 波段結束後的常態售價
    list_anchor: { bundle: 10800 },   // 劃線原價（錨點）
    waves: [
      { starts_at: iso("2026-08-07T00:00:00+08:00"), ends_at: iso("2026-08-21T00:00:00+08:00"), prices: { bundle: 4299 } },
    ],
  };
  const tWave = new Date("2026-08-10T00:00:00+08:00");
  const tAfter = new Date("2026-10-05T00:00:00+08:00");
  it("listAnchor 取 list_anchor", () => expect(listAnchor("bundle", anchored)).toBe(10800));
  it("listAnchor 未設 → 回退 list_price", () => expect(listAnchor("bundle", settings)).toBe(5800));
  it("波段中：售價=波段價、劃線=錨點", () =>
    expect(salePhase(anchored, tWave).plans.bundle).toEqual({ price: 4299, originalPrice: 10800, isEarlyBird: true }));
  it("波段後 list 態：售價 7999、劃線 10800", () => {
    const p = salePhase(anchored, tAfter);
    expect(p.state).toBe("list");
    expect(p.plans.bundle).toEqual({ price: 7999, originalPrice: 10800, isEarlyBird: false });
  });
  it("波段中夾擠用 anchor 防呆", () =>
    expect(currentPrice("bundle", { ...anchored, waves: [{ starts_at: anchored.waves[0].starts_at, ends_at: anchored.waves[0].ends_at, prices: { bundle: 99999 } }] }, tWave)).toBe(10800));
});

describe("isClassroomOpen / isPresale（沿用）", () => {
  it("開課前鎖", () => expect(isClassroomOpen(settings, tW1)).toBe(false));
  it("開課後開", () => expect(isClassroomOpen(settings, tList)).toBe(true));
  it("override locked", () => expect(isClassroomOpen({ ...settings, lock_override: "locked" }, tList)).toBe(false));
  it("isPresale 開課前 true", () => expect(isPresale(settings, tW1)).toBe(true));
  it("isClassroomOpen override 'open' 一律開", () => expect(isClassroomOpen({ ...settings, lock_override: "open" }, tW1)).toBe(true));
  it("isClassroomOpen null settings → 鎖", () => expect(isClassroomOpen(null, tList)).toBe(false));
  it("isClassroomOpen 無 open_at → 鎖", () => expect(isClassroomOpen({ ...settings, open_at: null }, tList)).toBe(false));
  it("isClassroomOpen now==open_at → 開", () => expect(isClassroomOpen(settings, new Date(settings.open_at))).toBe(true));
  it("isPresale 開課後 false", () => expect(isPresale(settings, tList)).toBe(false));
});

import { getFanPlan, validateFanPlan, isClassroomLockExempt } from "./sale.js";
import { FAN_PRICE, FAN_DIRECT_PRICE, FAN_PROOF_DEADLINE } from "./fan-proof.js";

describe("isClassroomLockExempt", () => {
  it("工具頁段（含子路徑）豁免", () => {
    for (const p of ["/classroom/login", "/classroom/account", "/classroom/reset-password", "/classroom/certificate", "/classroom/account/x"]) {
      expect(isClassroomLockExempt(p)).toBe(true);
    }
  });
  it("課程內容頁不豁免（fail-closed）", () => {
    for (const p of ["/classroom", "/classroom/", "/classroom/lesson/1", "/classroom/video", "/"]) {
      expect(isClassroomLockExempt(p)).toBe(false);
    }
  });
  it("前綴詐騙不豁免（/classroom/loginx 非 login）", () => {
    expect(isClassroomLockExempt("/classroom/loginx")).toBe(false);
    expect(isClassroomLockExempt("/classroomlogin")).toBe(false);
  });
  it("非字串不爆", () => {
    expect(isClassroomLockExempt(null)).toBe(false);
    expect(isClassroomLockExempt(undefined)).toBe(false);
  });
});

describe("getFanPlan", () => {
  it("完整值原樣正規化", () => {
    const s = { fan_plan: { enabled: false, deadline: "2026-09-03T23:59:59+08:00", proof_price: 3299, direct_price: 3799, proof_enabled: false, proof_discount: 500 } };
    expect(getFanPlan(s)).toEqual({ enabled: false, deadlineMs: Date.parse("2026-09-03T23:59:59+08:00"), proofPrice: 3299, directPrice: 3799, proofEnabled: false, proofDiscount: 500 });
  });
  it("空物件 → 全部 fallback（enabled／proof_enabled 預設 true、折抵預設 300）", () => {
    expect(getFanPlan({ fan_plan: {} })).toEqual({ enabled: true, deadlineMs: FAN_PROOF_DEADLINE, proofPrice: FAN_PRICE, directPrice: FAN_DIRECT_PRICE, proofEnabled: true, proofDiscount: 300 });
  });

  // 憑證折抵獨立於粉絲直購的截止日：直購關了，憑證還能開
  it("fanProofActive 只看 proof_enabled，不看 deadline", () => {
    const past = { fan_plan: { enabled: true, deadline: "2020-01-01T00:00:00+08:00" } };
    expect(fanCouponActive(past, new Date("2026-09-17T10:00:00+08:00"))).toBe(false);
    expect(fanProofActive(past)).toBe(true);
    expect(fanProofActive({ fan_plan: { proof_enabled: false } })).toBe(false);
  });
  it("null settings 不爆、enabled 預設 true", () => {
    expect(getFanPlan(null).enabled).toBe(true);
  });
  it("壞 deadline 字串 → fallback 預設", () => {
    expect(getFanPlan({ fan_plan: { deadline: "not-a-date" } }).deadlineMs).toBe(FAN_PROOF_DEADLINE);
  });
});

describe("validateFanPlan", () => {
  const good = { enabled: true, deadline: "2026-08-06T23:59:59+08:00", proof_price: 3499, direct_price: 3999 };
  it("合法 → ok", () => { expect(validateFanPlan(good)).toEqual({ ok: true }); });
  it("enabled 非 boolean → error", () => { expect(validateFanPlan({ ...good, enabled: "yes" }).ok).toBe(false); });
  it("deadline 不可 parse → error", () => { expect(validateFanPlan({ ...good, deadline: "x" }).ok).toBe(false); });
  it("proof_price 非正整數 → error", () => { expect(validateFanPlan({ ...good, proof_price: 0 }).ok).toBe(false); });
  it("proof > direct → error", () => { expect(validateFanPlan({ ...good, proof_price: 4000 }).ok).toBe(false); });
});

describe("fanPlanCoupon", () => {
  it("enabled → FAN3999 active、價=直購價、固定價券、bundle", () => {
    expect(fanPlanCoupon({ enabled: true, direct_price: 3999 })).toEqual({
      code: "FAN3999", name: "粉絲直購", type: "price", value: 3999,
      plan: "bundle", usage_limit: null, status: "active",
    });
  });
  it("disabled → 券 disabled", () => {
    expect(fanPlanCoupon({ enabled: false, direct_price: 3999 }).status).toBe("disabled");
  });
});

describe("fanCouponActive / salePhase.fanPlan.enabled（粉絲方案截止＝整個方案關）", () => {
  const fanSettings = { ...settings, fan_plan: { enabled: true, deadline: "2026-09-09T23:59:59+08:00", proof_price: 3699, direct_price: 3999 } };
  it("截止前可用", () => {
    expect(fanCouponActive(fanSettings, new Date("2026-09-09T12:00:00+08:00"))).toBe(true);
    expect(salePhase(fanSettings, new Date("2026-09-09T12:00:00+08:00")).fanPlan.enabled).toBe(true);
  });
  it("截止瞬間之後不可用，首頁 fanPlan.enabled 同步變 false", () => {
    expect(fanCouponActive(fanSettings, new Date("2026-09-10T00:00:00+08:00"))).toBe(false);
    expect(salePhase(fanSettings, new Date("2026-09-10T00:00:00+08:00")).fanPlan.enabled).toBe(false);
  });
  it("後台停用時即使未截止也不可用", () => {
    const off = { ...fanSettings, fan_plan: { ...fanSettings.fan_plan, enabled: false } };
    expect(fanCouponActive(off, new Date("2026-09-01T00:00:00+08:00"))).toBe(false);
  });
  it("fanPlanCoupon 代碼與常數一致", () => {
    expect(fanPlanCoupon({ enabled: true, direct_price: 3999 }).code).toBe(FAN_COUPON_CODE);
  });
});

// 購買確認信「購買方案」欄：顯示買家當下用什麼身分／價格買的
describe("purchasePhaseLabel", () => {
  const settings = { waves: [{ starts_at: "2026-09-14T00:00:00+08:00", ends_at: "2026-09-25T00:00:00+08:00", prices: { bundle: 4299 } }] };
  it("粉絲券（直購 FAN3999 與憑證 FAN-XXXX）→ 粉絲優惠", () => {
    expect(purchasePhaseLabel({ couponCode: "FAN3999", createdAt: "2026-09-15T10:00:00+08:00", settings })).toBe("粉絲優惠");
    expect(purchasePhaseLabel({ couponCode: "fan-ab12cd34", createdAt: "2026-11-05T10:00:00+08:00", settings })).toBe("粉絲優惠");
  });
  it("下單當下有波段 → 早鳥期間", () => {
    expect(purchasePhaseLabel({ createdAt: "2026-09-20T10:00:00+08:00", settings })).toBe("早鳥期間");
  });
  it("波段結束後 → 正式售價；沒有時間或設定也回正式售價（不亂猜）", () => {
    expect(purchasePhaseLabel({ createdAt: "2026-11-05T10:00:00+08:00", settings })).toBe("正式售價");
    expect(purchasePhaseLabel({})).toBe("正式售價");
    expect(purchasePhaseLabel({ createdAt: "not-a-date", settings })).toBe("正式售價");
  });
});
