...
import { aesEncrypt, makeHashInfo, parsePayuniCallback, payuniTrade, hashEqual, interpretPayment } from "./payuni.js";
...
describe("hashEqual（定值時間比對）", () => {
  it("相同字串 → true", () => {
    expect(hashEqual("ABCDEF123", "ABCDEF123")).toBe(true);
  });
  it("不同字串 → false", () => {
    expect(hashEqual("ABCDEF123", "ABCDEF124")).toBe(false);
  });
  it("長度不同 → false（不丟例外）", () => {
    expect(hashEqual("ABC", "ABCDEF")).toBe(false);
  });
  it("非字串 → false", () => {
    expect(hashEqual(null, "ABC")).toBe(false);
    expect(hashEqual("ABC", undefined)).toBe(false);
  });
});

describe("interpretPayment（notify 付款結果解讀）", () => {
  it("查無對應訂單 → known=false", () => {
    expect(interpretPayment(null, "3999")).toMatchObject({ known: false });
  });
  it("金額相符 → amountMismatch=false", () => {
    const r = interpretPayment({ status: "pending", amount: 3999 }, "3999");
    expect(r).toMatchObject({ known: true, paidAmt: 3999, amountValid: true, orderAmount: 3999, amountMismatch: false });
  });
  it("金額不符 → amountMismatch=true", () => {
    expect(interpretPayment({ amount: 3999 }, "3000").amountMismatch).toBe(true);
  });
  it("TradeAmt 非數字 → amountValid=false、不誤判 mismatch", () => {
    const r = interpretPayment({ amount: 3999 }, "abc");
    expect(r.amountValid).toBe(false);
