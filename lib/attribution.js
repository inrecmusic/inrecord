...
// 系統信件連結的 UTM：所有從信件導回站內的連結都走這裡，避免哪封信忘了加就永遠追不到來源。
// campaign 用信件種類（trial／purchase／launch／abandoned_recovery／newsletter 的草稿代號）。
export function withUtm(url, campaign, { source = "email", medium = "email" } = {}) {
  if (typeof url !== "string" || !url) return url;
  if (!campaign) return url;
  const [base, hash = ""] = url.split("#");
  const sep = base.includes("?") ? "&" : "?";
  const q = `utm_source=${encodeURIComponent(source)}&utm_medium=${encodeURIComponent(medium)}&utm_campaign=${encodeURIComponent(campaign)}`;
