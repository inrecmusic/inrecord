...
describe("mergeStudents", () => {
  ...
  it("paid 旗標：手動開通單（source='manual'）不算付費；同一人另有真實付款單才算", () => {
    const r = mergeStudents({
      orders: [
        { email: "test@x.com", plan: "bundle", status: "paid", source: "manual", amount: 0, created_at: "2026-09-04T00:00:00Z" },
        { email: "both@x.com", plan: "bundle", status: "paid", source: "payuni", amount: 3999, created_at: "2026-09-01T00:00:00Z" },
        { email: "both@x.com", plan: "bundle", status: "paid", source: "manual", amount: 0, created_at: "2026-09-04T00:00:00Z" }, // 最新一筆是手動
      ],
    });
    const by = Object.fromEntries(r.map(x => [x.email, x]));
    expect(by["test@x.com"].paid).toBe(false);
    expect(by["test@x.com"].purchased).toBe(true); // 仍列為學員（有訂單），只是不算付費
    expect(by["both@x.com"].paid).toBe(true);
  });
...
describe("countPaidBuyers", () => {
  ...
  it("後台手動開通（source='manual'、amount 0）不算購買者：只有真的付過錢的來源才算", () => {
    expect(countPaidBuyers([{ email: "m@x.com", status: "paid", source: "manual", amount: 0 }])).toBe(0);
    expect(countPaidBuyers([
      { email: "m@x.com", status: "paid", source: "manual", amount: 0 },
      { email: "m@x.com", status: "paid", source: "payuni", amount: 3999 }, // 同一人另有真實付款
      { email: "c@x.com", status: "paid", source: "concert", amount: 3699 },
    ])).toBe(2);
  });
