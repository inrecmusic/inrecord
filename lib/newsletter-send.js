...
import { listLeadEmails } from "./brevo-contacts.js";
...
// 依對象撈出收件 email：
//   buyers           → 已付款訂單 ∪ 已開通(enrollments)
//   buyers_early     → 上面那份裡「9/2 前購課／開通」的（音樂會預購、早鳥）
//   buyers_standard  → 上面那份裡「9/2 當天及之後」的
//   registered → Supabase Auth 使用者 email（分頁取完）
//   leads      → Brevo 潛客清單（首頁留信箱），再排除已購買者——買了課就別再收「還沒買嗎」
// 三種對象最後都排除 newsletter_unsubscribes 內的 email（信中「取消訂閱」按鈕寫入）。
// 已購課者的「首次購買／開通時間」與早鳥覆寫，供 9/2 前後分群使用。
// 判定與教室的搶先看分層同源（lib/early-access.js），兩邊不會各說各話：
// 取該 email 所有 paid 訂單與開通紀錄的最早時間；enrollments.early_override 優先於時間。
// 支援的群發對象（唯一真相）。新增對象時只改這裡與 gatherAudienceEmails，
// 路由的白名單與後台標籤都由此衍生 —— 先前白名單漏改，導致新對象的群發被擋在
// bad_audience、一封都寄不出去（fail-closed，沒有半套寄送，但使用者只看到一個錯誤碼）。
export const AUDIENCES = ["buyers", "buyers_early", "buyers_standard", "registered", "leads"];

export async function buildBuyerCohorts(supabase) {
  const [orders, enrollments] = await Promise.all([
    selectAll(supabase, "orders", (q) => q.select("email, grant_email, created_at").eq("status", "paid")),
    selectAll(supabase, "enrollments", (q) => q.select("email, enrolled_at, early_override")),
  ]);
  ...
  for (const o of orders) { add(o.email, o.created_at); add(o.grant_email, o.created_at); }
  for (const e of enrollments) {
    add(e.email, e.enrolled_at);
    if (e.early_override) override.set(norm(e.email), e.early_override);
  }
  return {
    isEarly(email) {
      const k = norm(email);
      const ov = override.get(k);
      if (ov === "early") return true;
      if (ov === "standard") return false;
      return isEarlyAccess({ orderTimes: times.get(k) || [] });
    },
  };
}

export async function gatherAudienceEmails(supabase, audience, { listLeads = listLeadEmails } = {}) {
  // 9/2 切點分群：先用 buyers 的同一套聯集與排除規則撈出全名單，再依首次購買時間切兩半。
  // 刻意不另外寫一份查詢 —— 兩邊規則一旦分岔，就會有人兩封都收到或兩封都收不到。
  if (audience === "buyers_early" || audience === "buyers_standard") {
    const [all, cohorts] = await Promise.all([
      gatherAudienceEmails(supabase, "buyers", { listLeads }),
      buildBuyerCohorts(supabase),
    ]);
    const wantEarly = audience === "buyers_early";
    return all.filter((e) => cohorts.isEarly(e) === wantEarly);
  }
  if (audience === "buyers") {
  ...
  if (audience === "leads") {
    const [leads, bought] = await Promise.all([listLeads(), gatherAudienceEmails(supabase, "buyers")]);
    const boughtSet = new Set(bought);
    return excludeUnsubscribed(supabase, dedupeEmails(leads).filter((e) => !boughtSet.has(e)));
  }
