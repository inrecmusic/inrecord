...
async function amegoPost(endpoint, dataObj) {
  ...
  const res = await fetch(`${process.env.AMEGO_API_URL}${endpoint}`, {
    ...
    // 與下方 g0v 統編查詢一致：Amego 卡住時 5 秒放棄，由呼叫端的 catch 降級放行；
    // 否則 checkout 會跟著卡到平台把函式砍掉，手機載具的結帳整個停住。
...
export async function verifyCarrier(barcode) {
  ...
  try {
    ...
    // Amego 成功碼可能回數字 0 或字串 "0"（與 parseAmegoResult 一致）；只比數字會把有效條碼誤擋
    if (r.code === 0 || r.code === "0") return { valid: true };
