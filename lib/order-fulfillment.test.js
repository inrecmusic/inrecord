...
import { needsFulfillment, needsInvoice, autoInvoiceEnabled } from "./order-fulfillment.js";
...
describe("autoInvoiceEnabled（自動開票開關，fail-safe 預設關）", () => {
  it("AUTO_INVOICE=on → 開啟自動開票", () => {
    expect(autoInvoiceEnabled({ AUTO_INVOICE: "on" })).toBe(true);
  });

  it("未設或其他值 → 關閉（發票改人工開立）", () => {
    expect(autoInvoiceEnabled({})).toBe(false);
    expect(autoInvoiceEnabled({ AUTO_INVOICE: "off" })).toBe(false);
    expect(autoInvoiceEnabled({ AUTO_INVOICE: "true" })).toBe(false);
    expect(autoInvoiceEnabled({ AUTO_INVOICE: "" })).toBe(false);
    expect(autoInvoiceEnabled(undefined)).toBe(false);
  });
});
