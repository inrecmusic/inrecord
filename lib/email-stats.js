...
// sendNewsletterEmail 自這天起才帶 tags:[kind]。更早寄的信在 Brevo 事件裡沒有 tag，
// 只能用「收件人＋時間」推算 → UI 要誠實標註為推算值。
// ⚠️ 這是「帶 tag 的程式碼上正式站的日期」，不是寫程式的日期。改部署日要同步改這裡。
export const TAG_SINCE_TW_DAY = "2026-09-13";

// 這組寄送是否帶得到 tag（＝可要求事件必須帶對的 tag，精準排除其他類型信件）。
export function hasTag(group) {
  return String(group?.dateTW || "") >= TAG_SINCE_TW_DAY;
}
...
export function attributeEvents(index, events = []) {
  ...
  for (const ev of events) {
    ...
    if (!em || !Number.isFinite(at)) continue;
    const list = index.get(em);
    if (!list) continue;
    const hit = list.find((s) => at >= s.at - GRACE_MS);
    if (!hit || at - hit.at > MAX_AGE_MS) continue;
...
//
// requireTag：這組寄送時已經會帶 tag → 事件必須帶對的 tag 才採計（沒帶 tag 的一律不算）。
// 這是唯一能把「同一位收件人同期收到、且我們沒有納入時間軸的信」擋掉的機制。
// 舊資料（requireTag=false）只能寬鬆採計，由 UI 標註為推算值。
//
...
export function summarizeGroup(group, events = [], { requireTag = false } = {}) {
  ...
  for (const ev of events) {
    ...
    if (kind) {
      if (requireTag && !tags.includes(kind)) continue;       // 精準模式：沒帶對 tag 就不是這封
      if (!requireTag && tags.length && !tags.includes(kind)) continue; // 推算模式：帶了別人的 tag 才排除
    }
    const type = ev?.event;
