...
export async function runLaunchNotify(supabase, { sendLaunchEmail, now = new Date() }) {
  ...
  const { data: orders, error: ordersError } = await supabase
    .from("orders").select("email").eq("status", "paid");
  if (ordersError) {
    // 已搶佔旗標但撈不到名單 → 還原 launch_notified_at，讓下次 cron/手動可重試，
    // 否則旗標永遠停在「已通知」而其實一封都沒寄（不可逆）。
    await supabase.from("sale_settings").update({ launch_notified_at: null }).eq("id", "default");
    throw new Error("launch-notify: 撈付款名單失敗，已還原旗標: " + ordersError.message);
  }
