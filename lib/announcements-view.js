...
// 提示條要顯示哪一則：有「置頂」就常駐顯示最新那則置頂（persistent，沒有 × 可關，關過也照樣出現）；
// 沒有置頂才退回「最新一則、可關」的行為。
export function pickStrip(sorted, state, dismissedId) {
  const list = (sorted || []).slice().sort((a, b) => ts(b.created_at) - ts(a.created_at));
  const pinned = list.find((a) => a.pinned);
  if (pinned) return { ...pinned, unread: isUnread(pinned, state), persistent: true };
  const top = list[0];
