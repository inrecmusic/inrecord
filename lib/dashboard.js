...
const PAY_TYPE_LABELS = {
  // PayUni notify 的 PaymentType 數字碼，依官方 UPP 文件回傳參數表（docs.payuni.com.tw）：
  // 1 信用卡、2 ATM、3 超商代碼／條碼、5 貨到付款、6 icash Pay、7 AFTEE 後支付、9 LINE Pay、10 宅配到付、11 街口支付。
  // 1／2／7 正式站已實際收到過。
  "1": "信用卡", "2": "ATM 轉帳", "3": "超商代碼", "5": "貨到付款", "6": "icash Pay",
  "7": "AFTEE 後支付", "9": "LINE Pay", "10": "宅配到付", "11": "街口支付",
  // 相容其他可能的字串型別
...
const SOURCE_LABELS = {
  concert: "音樂會現場", wordpress: "碩樂現場", manual: "手動開通", payuni: "線上購買",
};

// 一筆訂單的「付款方式／來源」標籤：有 pay_type 用付款方式（未知數字碼標「其他（碼）」），
// 否則退回來源（外部站台如音樂會現場無 pay_type）。供儀表板與對帳彙整共用，確保一致。
