...
export async function handleWoocommerceWebhook({ rawBody, signature, secret, productMap, supabase, source = "wordpress", sendEmail, removeLead }) {
  if (!secret) {
  ...
  // 買了課就退出 Brevo 潛客名單（與官網直購 notify 同一個原則：名單裡只留沒買過的人）。
  // 現場／WordPress 成交走這條路，以前不會移除，先留過信箱的人買了還是會收到「還在考慮嗎」。
  // best-effort：失敗只記 log，絕不影響 webhook 回應（訂單已入名單為準）。
  if (removeLead) {
    try {
      await removeLead(course.email);
    } catch (e) {
      console.error("[woo webhook] 移除潛客失敗（不影響入名單）", e?.message || e);
    }
  }
  ...
  if (sendEmail) {
    try {
      ...
      // claim 出錯（DB 抖動等）≠「已寄過」：後者 data=null,error=null 正常略過；前者要記 email_error，
      // 否則名單看起來像「還沒寄」卻永不重試（webhook 回 200 不重送、旗標仍 NULL、無錯誤訊號）。
      if (claimErr) {
        console.error("[woo webhook] presale claim 失敗", course.merTradeNo, claimErr.message);
        await supabase.from("orders")
          .update({ email_error: `presale_claim_failed: ${claimErr.message}` })
          .eq("mer_trade_no", course.merTradeNo)
          .is("presale_email_sent_at", null);
      } else if (claimed) {
        let result;
