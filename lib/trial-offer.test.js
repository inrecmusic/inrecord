...
describe("trialOffer", () => {
  ...
  it("波段全部結束 → 顯示官網牌價（與首頁一致），沒有截止倒數", () => {
    const o = offerAt("2026-11-05T12:00:00+08:00", { ...settings, fan_plan: { ...settings.fan_plan, enabled: false } });
    expect(o.mode).toBe("list");
    expect(o.price).toBe(13800);
    expect(o.deadlineLabel).toBeUndefined();
  });

  it("連方案價都取不到才回 none（不報價，也不報錯價）", () => {
