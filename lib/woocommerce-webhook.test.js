...
function call(rawBody, { secret = SECRET, sig, supabase, source, sendEmail, removeLead } = {}) {
...
describe("handleWoocommerceWebhook", () => {
  ...
  it("已付款課程訂單 → 把買家移出 Brevo 潛客名單（信箱已正規化小寫）", async () => {
    const removed = [];
    const { res } = await call(orderPayload(), { removeLead: async (e) => { removed.push(e); } });
    expect(res.status).toBe(200);
    expect(removed).toEqual(["fan@example.com"]);
  });

  it("移除潛客失敗不影響入名單（best-effort）", async () => {
    const { res, calls } = await call(orderPayload(), { removeLead: async () => { throw new Error("brevo down"); } });
    expect(res.status).toBe(200);
    expect(calls[0].row.mer_trade_no).toBe("WC727"); // 訂單仍有寫進名單
  });

  it("非課程訂單不會去動潛客名單", async () => {
    const removed = [];
    const body = JSON.stringify({ id: 1, status: "processing", billing: { email: "a@x.com" }, line_items: [{ product_id: 999, name: "杯墊" }] });
    const { res } = await call(body, { removeLead: async (e) => { removed.push(e); } });
    expect(res.status).toBe(200);
