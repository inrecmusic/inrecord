...
import { addLeadContact, listLeadEmails, removeLeadContact, removeLeadContacts } from "./brevo-contacts.js";
...
describe("brevo-contacts（官網潛客清單）", () => {
  ...
  it("removeLeadContacts：多個信箱去重、略過空值，各打一次", async () => {
    const f = vi.fn().mockResolvedValue(jsonRes(201, {}));
    const r = await removeLeadContacts(["a@x.com", "b@x.com", "a@x.com", null, ""], { fetchImpl: f });
    expect(r).toEqual({ attempted: 2, removed: 2 });
    expect(f.mock.calls.map((c) => JSON.parse(c[1].body).emails[0])).toEqual(["a@x.com", "b@x.com"]);
  });

  it("removeLeadContacts：單一字串也收；失敗不丟錯（購買流程不可被名單操作中斷）", async () => {
    const f = vi.fn().mockResolvedValue(jsonRes(500, {}));
    await expect(removeLeadContacts("a@x.com", { fetchImpl: f })).resolves.toEqual({ attempted: 1, removed: 0 });
  });

  it("removeLeadContacts：沒有信箱時不打 API", async () => {
    const f = vi.fn();
    expect(await removeLeadContacts([], { fetchImpl: f })).toEqual({ attempted: 0, removed: 0 });
    expect(f).not.toHaveBeenCalled();
  });
});
