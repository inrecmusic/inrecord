...
// 定值時間比對兩個字串（簽章/HashInfo 用，避免時序側通道）。長度不同或非字串 → false。
...
// 解讀 notify 付款結果（純函式）：是否為已知訂單、付款金額是否有效、是否與下單金額相符。
// notify 不應信任回呼金額去覆寫訂單；以此判斷「未知單應略過」「金額不符要告警」。
export function interpretPayment(prior, tradeAmt) {
  const paidAmt = Number(tradeAmt);
  const amountValid = Number.isFinite(paidAmt);
  const known = !!prior;
  const orderAmount = prior?.amount ?? null;
  const amountMismatch = known && amountValid && orderAmount != null && paidAmt !== orderAmount;
  return { known, paidAmt, amountValid, orderAmount, amountMismatch };
}
...
export function parsePayuniCallback(encryptInfo, hashInfo, key, iv) {
  ...
  try {
    if (hashInfo && !hashEqual(makeHashInfo(encryptInfo, key, iv), hashInfo)) {
...
export async function payuniTrade(path, encryptFields) {
  ...
  // 驗證回傳 Hash（防竄改）
  if (json.HashInfo && !hashEqual(makeHashInfo(json.EncryptInfo, hashKey, hashIV), json.HashInfo)) {
