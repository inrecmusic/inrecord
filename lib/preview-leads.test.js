...
describe("buildLeadPatch", () => {
  it("沒帶 status → patch 不含 status（不覆寫既有狀態），也不含 id；任意欄位不透傳", () => {
    const p = buildLeadPatch({ id: "L1", note: "hi" }, NOW);
    expect(p).not.toHaveProperty("status");
    expect(p).not.toHaveProperty("id");
    expect(p).not.toHaveProperty("note"); // 白名單：任意欄位不再 spread（防 mass-assignment）
  ...
  it("demo_opened_at 一律由伺服器設為 now（忽略呼叫端帶入值，防竄改）", () => {
