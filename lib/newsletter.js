...
const S = {
  ...
  p:  'margin:0 0 14px;',
  ul: 'margin:8px 0 18px;padding-left:4px;list-style:none;',
  // Gmail 不吃 li::before，改用真的 › 字元 span
...
// 文字一律跳脫。全部樣式 inline（不靠 <style>，才能在 Gmail 一致呈現）。
export function mdToHtml(md) {
  ...
  const flush = () => {
    if (list.length) {
      out.push(`<ul style="${S.ul}">` + list.map((li) =>
        `<li style="${S.li}"><span style="${S.mark}">›</span>${inline(li)}</li>`).join("") + "</ul>");
      list = [];
    }
  };
  for (const raw of lines) {
    ...
    if (line === "---") { flush(); out.push(`<hr style="${S.hr}">`); continue; }
    ...
    if (h) { flush(); const lvl = h[1].length; out.push(`<h${lvl} style="${S["h"+lvl]}">${inline(h[2])}</h${lvl}>`); continue; }
    ...
    <tr><td align="center" style="padding:28px 16px;">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" style="width:600px;max-width:100%;border-radius:20px;overflow:hidden;box-shadow:0 12px 40px rgba(15,23,42,.10);">
        ...
        <tr><td align="center" bgcolor="#0f172a" style="background-color:#0f172a;padding:38px 40px 34px;">
          ...
          <h1 style="margin:0;color:#ffffff;font-size:23px;line-height:1.5;font-weight:800;letter-spacing:-.01em;">${subj}</h1>
        </td></tr>
        <!-- 白色內文 -->
