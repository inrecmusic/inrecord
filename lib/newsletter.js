...
function inline(s) {
  ...
  return esc(s)
    ...
    // 數字與中文之間的空格改成不斷行空格：站內文案慣例是「至少 3 年」「10 章節」這樣加空格，
    // 但那個空格是合法斷行點，實際收到的信出現過「至少 3」換行「年。」。視覺寬度不變。
    .replace(/(\d)[ \t]+([\u4e00-\u9fff])/g, "$1\u00a0$2")
    .replace(/([\u4e00-\u9fff])[ \t]+(\d)/g, "$1\u00a0$2");
}
...
// 中文斷句。
// ⚠️ 內文不可用 keep-all + overflow-wrap:anywhere：Gmail 的欄寬比這裡宣告的窄，詞不斷行的長中文
// 句塞不下就會觸發「緊急斷行」，而緊急斷行無視禁則——實際收到的信出現過句號跑到行首。
// 內文改回中文預設的逐字斷行（word-break:normal），行首標點交給 line-break:strict 擋，
// 這組不會溢出、也不會觸發緊急斷行。標題短，仍用 keep-all 保住專有名詞不被切開。
// !important 是必要的：Gmail 會對信件內容注入自己的 word-break 規則避免溢出，
// 那個規則會蓋掉我們的設定並造成行首標點。內嵌樣式加 !important 才贏得過。
const CJK = 'word-break:normal !important;line-break:strict !important;';
const CJK_TITLE = 'word-break:keep-all !important;line-break:strict !important;overflow-wrap:break-word;';
const S = {
  h1: 'font-size:23px;font-weight:800;color:#0f172a;margin:24px 0 12px;letter-spacing:-.01em;' + CJK_TITLE,
  h2: 'font-size:19px;font-weight:800;color:#2563eb;margin:26px 0 10px;padding-left:11px;border-left:3px solid #2563eb;line-height:1.4;' + CJK_TITLE,
  h3: 'font-size:18px;font-weight:800;color:#1e293b;margin:18px 0 8px;' + CJK_TITLE,
  p:  'margin:0 0 14px;' + CJK,
  // 清單用 <div> 而非 <ul>/<li>：Gmail 不吃 <ul> 的 list-style:none，會冒出雙重圓點。
  ul: 'margin:8px 0 18px;',
  // 懸掛縮排＋真的 › 字元 span（Gmail 不吃 ::before）
  li: 'margin:0 0 9px;padding-left:20px;text-indent:-20px;' + CJK,
  ...
  table: 'width:100%;border-collapse:collapse;font-size:15.5px;margin:6px 0 16px;',
...
export function mdToHtml(md, { plain = false } = {}) {
  ...
  const out = [];
  let list = [];
  let olist = []; // 連續「1. 」「2. 」編號行 → <ol>（法律條文用）
  let tbl = [];  // 收集連續的 | 表格列
  let tl = null; // 收集 :::timeline 區塊列；非 null 代表正在區塊內
  let para = []; // 收集連續內文行，一起輸出成單一 <p>（行間用 <br>）
  ...
  const flushOl = () => {
    if (olist.length) {
      out.push(plain
        ? "<ol>" + olist.map((li) => `<li>${inline(li)}</li>`).join("") + "</ol>"
        : `<div style="${S.ul}">` + olist.map((li, i) =>
          `<div style="${S.li}"><span style="${S.mark}">${i + 1}.</span>${inline(li)}</div>`).join("") + "</div>");
      olist = [];
    }
  };
  // 連續的內文行併成同一個 <p>，行與行之間用 <br>：這樣「我在原稿斷的行」在信裡就是單純換行
  // （行距＝line-height），而不是每行各自成段、行行之間多出 14px 的段距。
  // 要真正分段請空一行。2026-09-24 調整；既有草稿若原本靠「行行成段」製造間距，會變得比較緊。
  const flushP = () => {
    if (!para.length) return;
    out.push(`<p${st("p")}>` + para.map(inline).join("<br>") + "</p>");
    para = [];
  };
  const flush = () => { flushP(); flushUl(); flushOl(); };
  for (const raw of lines) {
    ...
    // ![alt](url|寬度|left)：第三段可加 left 讓圖片靠左（預設置中）。照片型的圖靠左，
    // 和左邊的標題／內文對齊起來比置中好看；吉祥物這種裝飾圖維持置中。
    const img = line.match(/^!\[([^\]]*)\]\((https?:\/\/[^)\s|]+)(?:\|(\d{1,4}))?(?:\|(left))?\)$/);
    if (img) {
      flush();
      const w = img[3] ? Math.min(540, Number(img[3])) : 160;
      const align = img[4] === "left" ? "left" : "center";
      out.push(`<p style="text-align:${align};margin:8px 0 18px;"><img src="${esc(img[2])}" alt="${esc(img[1])}" width="${w}" style="width:${w}px;max-width:100%;height:auto;border:0;display:inline-block;" /></p>`);
      continue;
    }
    const h = line.match(/^(#{1,3})\s+(.*)$/);
    if (h) { flush(); const lvl = h[1].length; out.push(`<h${lvl}${st("h" + lvl)}>${inline(h[2])}</h${lvl}>`); continue; }
    const li = line.match(/^-\s+(.*)$/);
    if (li) { flushP(); flushOl(); list.push(li[1]); continue; }
    const oli = line.match(/^\d{1,3}\.\s+(.*)$/);
    if (oli) { flushP(); flushUl(); olist.push(oli[1]); continue; }
...
function wrap({ subject, bodyMd, siteUrl, footer }) {
  ...
  const subtitleHtml = subtitle
    ? `<p style="margin:12px 0 0;color:#aab6c8;font-size:16px;line-height:1.7;">${esc(subtitle)}</p>`
    : "";
...
/* Gmail 深色模式會把深色頁首反轉成淺色，白字 Logo 就看不清楚。宣告 color-scheme 後
   多數用戶端會跳過自動反轉；仍會反轉的（部分 Outlook）用 data-ogsc 補上原色。 */
[data-ogsc] .ir-head{background-color:#0f172a !important;}
[data-ogsc] .ir-head h1{color:#ffffff !important;}
[data-ogsc] .ir-body{background-color:#ffffff !important;color:#3a4657 !important;}</style></head>
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#eef2f7;">
    <tr><td align="center" style="padding:28px 16px;">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" style="width:600px;max-width:100%;border-radius:20px;overflow:hidden;box-shadow:0 12px 40px rgba(15,23,42,.10);">
        <!-- 深色頁首 -->
        <tr><td class="ir-head" align="center" bgcolor="#0f172a" style="background-color:#0f172a;padding:36px 30px 32px;">
          ...
          ${badgeHtml}<h1 style="margin:0;color:#ffffff;font-size:25px;line-height:1.42;font-weight:800;letter-spacing:-.01em;">${subj}</h1>
          ${subtitleHtml}
        </td></tr>
        <!-- 白色內文 -->
        <tr><td class="ir-body" bgcolor="#ffffff" style="background-color:#ffffff;padding:32px 30px 36px;color:#3a4657;font-size:18px;line-height:1.78;">
          ${body}
          <p style="color:#a6b0bd;font-size:14px;line-height:1.75;margin:32px 0 0;border-top:1px solid #f1f5f9;padding-top:18px;">${footer}</p>
        </td></tr>
      </table>
      ...
      <table role="presentation" cellpadding="0" cellspacing="0" border="0" align="center" style="margin:22px auto 0;">
        <tr><td align="center" style="color:#8a94a3;font-size:14.5px;line-height:2;font-family:inherit;">
          ...
          <span style="color:#c3cad4;">　·　</span>Facebook<span style="color:#c3cad4;">｜</span><a href="https://www.facebook.com/people/InRecord-%E9%9F%B3%E6%A8%82%E5%88%BB/61592320384384/" style="color:#64748b;text-decoration:underline;text-underline-offset:2px;">InRecord 音樂刻</a>
        </td></tr>
      </table>
    </td></tr>
  </table>
...
// reasonLine：收信理由。預設是學員／註冊會員（電子報群發用）；批次追單的收件人是「下了單還沒付款」的
// 消費者，既不是學員也不保證註冊過，用預設句等於對顧客講錯話，所以開放呼叫端覆寫。
export function renderNewsletterHtml({ subject, bodyMd, siteUrl, unsubscribeUrl, reasonLine }) {
  const base = reasonLine || "你收到這封信，是因為你是 InRecord 的學員／註冊會員。";
...
// 把內文裡指向自家站台的連結補上 UTM（campaign＝草稿代號），讓每封電子報的成交追得回來。
// 只動 markdown 連結的網址部分；已有 utm_source 的不覆寫；圖片網址（![...](...)）不碰。
export function tagNewsletterLinks(md, siteUrl, campaign) {
  if (typeof md !== "string" || !md || !campaign) return md;
  let host;
  try { host = new URL(siteUrl || "https://inrecordmusic.com").host; } catch { return md; }
  return md.replace(/(!?)\[([^\]]*)\]\((https?:\/\/[^)\s]+)\)/g, (full, bang, text, url) => {
    if (bang) return full;                        // 圖片不動
    if (/[?&]utm_source=/.test(url)) return full; // 已指定來源就尊重它
    let u;
    try { u = new URL(url); } catch { return full; }
    if (u.host !== host) return full;             // 外部連結不動
    const [base, hash = ""] = url.split("#");
    const sep = base.includes("?") ? "&" : "?";
    const q = `utm_source=newsletter&utm_medium=email&utm_campaign=${encodeURIComponent(campaign)}`;
    return `[${text}](${base}${sep}${q}${hash ? "#" + hash : ""})`;
  });
}
