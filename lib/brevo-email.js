...
// 電子報群發用：寄一封任意 subject/html 給單一收件人。回 { success, limitHit?, error? }。
// limitHit：402（額度/每日上限用罄）或 429（速率限制）→ 由 sendNewsletterBatch 據此停止群發。
export async function sendNewsletterEmail({ to, subject, html, kind = "newsletter" }) {
  const r = await (async () => {
  const apiKey = process.env.BREVO_API_KEY;
  const sender = process.env.BREVO_SENDER_EMAIL;
  if (!apiKey || !sender) return { success: false, skipped: true, error: "missing_brevo_config" };
  ...
  await recordEmail({ to, subject, kind, status: _logStatus(r), error: r.error });
  return r;
}
