...
describe("trackEvent", () => {
  ...
  it("Lead（留信箱）映射 GA4 generate_lead，Meta 帶 content_name", () => {
    trackEvent("Lead", { contentName: "newsletter" });
    expect(window.fbq).toHaveBeenCalledWith("track", "Lead", { content_name: "newsletter" });
    expect(window.gtag).toHaveBeenCalledWith("event", "generate_lead", expect.anything());
  });
  it("globals 未定義不丟錯", () => {
